...
class wxpay_jsapi
{
    ...
    public function paymentHtml($control = null)
    {
        ...
        $params['paySign'] = $sign;
        $this->config->orderAttach;
        // print_R($this->config->orderAttach);die;
        // @todo timestamp
        $jsonParams = json_encode($params);
        // print_R($jsonParams);die;
        return <<<EOB
<!DOCTYPE html>
<html>
<head>
    ...
    WeixinJSBridge.invoke(
        ...
        function(res) {
            ...
            if (res && res.err_msg == "get_brand_wcpay_request:ok") {
                ...
                //     errMsg = '您已取消本次交易或者此微信订单已过期。请重新下单！';
                // } else {
                //     errMsg = res.err_code+res.err_desc+res.err_msg;
                // }
...
EOB;
    ...
    public function getPrepayId()
    {
        ...
        $sign = $this->sign($data);
        $data['sign'] = $sign;
        // $logfile = BASE_PATH."/wxpay_.". date('Y-m-d') .'.log';
        
        // $logtxt = "\r\n";
        $result = $this->postXml('https://api.mch.weixin.qq.com/pay/unifiedorder', $data);
        // $logtxt .= "微信回调：".json_encode($result)."\r\n";
