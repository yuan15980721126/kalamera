...
class member_pointorderControl extends mobileMemberControl{
    public function __construct() {
        ...
        if (C('points_isuse') != 1 || C('pointprod_isuse') != 1){
            ...
            output_error(L('member_pointorder_unavailable'));
        }
    ...
    public function orderlistOp() {
        ...
        if (is_array($order_list) && count($order_list)>0){
            foreach ($order_list as $k => $v){
                $v['point_addtime'] =  date('Y-m-d H:i:s',$v['point_addtime']);
        ...
        // print_R($order_listnew);
        // foreach ($order_listnew as $key => $value) {
        //     $order_listnewp[$key]['point_addtime'] =  date('Y-m-d H:i:s',$value['point_addtime']);
        // }
        //信息输出
        $arr =array('order_list'=>$order_listnew);
        // print_R($arr);
        output_data($arr);
    ...
    public function cancel_orderOp(){
        $model_pointorder = Model('pointorder');
        //取消订单
        $data = $model_pointorder->cancelPointOrder($_GET['order_id'],$this->member_info['member_id']);
        if ($data['state']){
            showDialog(L('member_pointorder_cancel_success'),'index.php?model=member_pointorder','succ');
        }else {
            showDialog($data['msg'],'index.php?model=member_pointorder','error');
        }
    }
    /**
     * 确认收货
    ...
    public function receiving_orderOp(){
        ...
        if ($data['state']){
            showDialog(L('member_pointorder_confirmreceiving_success'),'index.php?model=member_pointorder','succ');
        }else {
            showDialog($data['msg'],'index.php?model=member_pointorder','error');
        }
    }
    ...
    public function order_infoOp(){
        ...
        if ($order_id <= 0){
            showDialog(L('member_pointorder_parameter_error'),'index.php?model=member_pointorder','error');
        }
        $model_pointorder = Model('pointorder');
        //查询兑换订单信息
        $where = array();
        ...
        if (!$order_info){
            showDialog(L('member_pointorder_record_error'),'index.php?model=member_pointorder','error');
        }
        //获取订单状态
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'pointorder':
                $menu_array = array(
                    1=>array('menu_key'=>'points',  'menu_name'=>'积分明细',    'menu_url'=>'index.php?model=member_points'),
                    2=>array('menu_key'=>'orderlist','menu_name'=>Language::get('member_pointorder_list_title'),    'menu_url'=>'index.php?model=member_pointorder&fun=orderlist')
                );
                break;
            case 'pointorderinfo':
                $menu_array = array(
                    1=>array('menu_key'=>'points',  'menu_name'=>'积分明细',    'menu_url'=>'index.php?model=member_points'),
                    2=>array('menu_key'=>'orderlist','menu_name'=>Language::get('nc_member_path_pointorder_list'),  'menu_url'=>'index.php?model=member_pointorder&fun=orderlist'),
