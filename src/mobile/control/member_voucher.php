...
class member_voucherControl extends mobileMemberControl {
    ...
    public function voucher_listOp() {
        ...
        //     $voucher_list[$key]['voucher_end_date'] = date("Y-m-d H:i:s",$value['voucher_end_date']);
        // }
     ...
     * 无效优惠券列表
     */
    // public function voucher_nullOp() {
    //     $param = $_POST;

    //     $model_voucher = Model('voucher');
    //     $data['voucher_state'] = 4;
    //     $where = array();
    //     $where['voucher_state'] = array('in','2,3,4');
     ...
     * 更改代金券
     */
    public function voucher_editOp() {

        $voucher_id = $_POST['voucher_id'];

        if(strstr($voucher_id,',')){
            $voucher_id = rtrim($voucher_id,',');
            // $messageid_str = "'".$voucher_id."'";
        }else{
       ...
       // print_r($messageid_str);
        // $voucherid_arr = explode(',',$voucher_id);
        // $messageid_str = '';
        // if (!empty($voucherid_arr)){
        ...
        $model_voucher = Model('voucher');
        //验证是否可领取代金券
        // $condition['voucher_id'] = array($messageid_str);
        $condition = array();
        if(strstr($voucher_id,',')){
            $condition['voucher_id'] = array('in',$voucher_id);
        }else{
        ...
        $data = array();
        $data['voucher_state'] = 5;
        $update = $model_voucher->editVoucher($data,$condition);
        if ($update){
            output_data('1');
        }else{
            output_error('删除失败');
