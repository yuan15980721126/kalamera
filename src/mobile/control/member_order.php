...
class member_orderControl extends mobileMemberControl {
    ...
    public function order_listOp() {
        ...
        if ($_POST['state_type'] != '') {
            $condition['order_state'] = str_replace(
                ...
                array(ORDER_STATE_NEW,ORDER_STATE_PAY,ORDER_STATE_SEND,ORDER_STATE_SUCCESS,ORDER_STATE_SUCCESS,ORDER_STATE_CANCEL), $_POST['state_type']);
        }
        ...
        foreach ($order_list_array as $value) {
            $order_pay_info = Model('order')->getOrderPayInfo(array('pay_sn'=> $value['pay_sn']));
            // print_r($order_pay_info);
            ...
            $value['pay_id'] = $order_pay_info['pay_id'];
            //商品图
            foreach ($value['extend_order_goods'] as $k => $goods_info) {
                ...
                $value['goods_arr'].= $goods_info['goods_id'].',';
                // $goods_arr .= $goods_info['goods_id'].',';
            }
            $value['goods_arr']=rtrim($value['goods_arr'],',');
    ...
    public function order_cancelOp() {
        ...
        $state_info = intval($_POST['state_info']);
        $condition = array();
        ...
        $result = $logic_order->changeOrderStateCancel($order_info,'buyer', $this->member_info['member_name'], $msg);
        if(!$result['state']) {
    ...
    public function order_infoOp() {
        ...
        $data['pay_sn'] =  $result['data']['order_info']['pay_sn'];
        $order_pay_info = Model('order')->getOrderPayInfo(array('pay_sn'=> $result['data']['order_info']['pay_sn']));
        $data['pay_id'] = $order_pay_info['pay_id'];
...
//         $data['refund_state'] = $result['data']['order_info']['refund_state'];
        ...
        foreach ($result['data']['order_info']['goods_list'] as $_k => $_v) {
            ...
            $data['goods_list'][$_k]['goods_arr'] = $_v['goods_id'];
            $goods_arr .= $_v['goods_id'].',';
            $goods_yuanname .= $_v['goods_name'].',';
        }

        $data['goods_arr'] = rtrim($goods_arr,',');
        $data['goods_yuanname'] = rtrim($goods_yuanname,',');
