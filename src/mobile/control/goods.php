...
class goodsControl extends mobileHomeControl{
    ...
    public function goods_listOp() {
        ...
        // echo "<pre>";
        // print_r($goods_list);
        $nav_link_list = Model('goods_class')->getGoodsClassNav($_GET['gc_id'], 0);

        // $nav_link_list[] = array('title' => $goods_info['goods_name']);
        $gc_name = end($nav_link_list);

        foreach ($goods_list as $key => $value) {
            $goods_evaluate_info = Model('evaluate_goods')->getEvaluateGoodsInfoByGoodsID($value['goods_id']);
            $good_percent = $goods_evaluate_info['good_percent'];
            $goods_list[$key]['good_percent'] =  $good_percent;
            $goods_list[$key]['gc_name'] =  $gc_name['title'];
            $goods_info = $model_goods->getGoodsInfoAndPromotionById($value["goods_id"]);
    ...
    //     // 验证商品是否存在
    //     if (empty($goods_info)) {
    //         output_error(L('goods_index_no_goods'));
    //     }
    ...
    public function save_consultOp(){
        ...
        if(!C('guest_comment') && !$_SESSION['member_id']){
            output_error(L('goods_index_goods_noallow'));
        }
        ...
        if(trim($_POST['goods_content'])== ""){
            output_error('咨询内容不能为空');
        }
        //表单验证
        // $result = chksubmit(true,C('captcha_status_goodsqa'),'num');
        // if (!$result){
        //     output_error(L('invalid_request'));
        // } elseif ($result === -11){
        //     output_error(L('invalid_request'));
        // }elseif ($result === -12){
        //     output_error(L('wrong_checkcode'));
        // }
        if (process::islock('commit')){
            output_error(L('nc_common_op_repeat'));
        }else{
            process::addprocess('commit');
        }
        $model_mb_user_token = Model('mb_user_token');
        $mb_user = $model_mb_user_token->getMbUserTokenInfoByToken($_POST['key']);
        // print_R($mb_user);die;
        
        if($mb_user['member_id']){
            //查询会员信息
            $member_model = Model('member');
            $member_info = $member_model->getMemberInfo(array('member_id'=>$mb_user['member_id']));
            // print_R($member_info);
            if(empty($member_info) || $member_info['is_allowtalk'] == 0){
                output_error('您没有咨询权限');
            }
        }
        //判断商品编号的存在性和合法性
        $goods = Model('goods');
        ...
        $input['member_id']         = intval($mb_user['member_id']) > 0?$mb_user['member_id']:0;
        $input['member_name']       = $mb_user['member_name']?$mb_user['member_name']:'';
