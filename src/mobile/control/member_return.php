    ...
    public function ship_postOp(){
        ...
        if(!empty($return)) {
            ...
            if ($state) {
                ...
                output_error('退款退货申请,发货Save failed');
            }
        } else {
            output_error('参数错误');
        }
    ...
    public function delay_postOp(){
        ...
        if(!empty($return)) {
            ...
            if ($state) {
                ...
                output_error('退款退货申请,延迟收货Save failed');
            }
        } else {
            output_error('参数错误');
        }
    ...
    public function get_return_listOp() {
        ...
        if(!empty($list) && is_array($list)) {
            ...
            foreach($list as $k => $v) {
                ...
                $val['goods_state_v'] = $v['goods_state'];//物流状态:1为To be shipped,2为待收货,3为未收到,4为已收货
                $val['ship_state'] = '0';
                $val['delay_state'] = '0';
                ...
                $yuanlai = Model('order')->getOrderGoodsInfo($condition);
                // print_R($yuanlai);
                $goods_arr = $yuanlai['goods_id'];
                $val['goods_arr'] = $goods_arr;
                $val['yuanlaiorder_time'] = date("Y-m-d H:i:s",$yuanlai['add_time']);
                $val['yuanlaigoods_id'] = $yuanlai['goods_id'];
