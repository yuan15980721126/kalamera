...
function update_flex(){
	//加载统计列表
    $("#glist").flexigrid({
        url: 'index.php?model=stat_marketing&fun=get_grouplist_xml&'+$("#formSearch").serialize(),
        colModel : [
    ...
    $("#flexigrid").flexigrid({
        url: 'index.php?model=stat_marketing&fun=get_groupgoods_xml&'+$("#formSearch").serialize(),
        colModel : [
            {display: '操作', name : 'operation', width : 60, sortable : false, align: 'center', className: 'handle-s'},
...
$(function () {
	...
	$("[name='searchweek_month']").change(function(){
		...
		$.getJSON('<?php echo ADMIN_SITE_URL?>/index.php?model=common&fun=getweekofmonth',{y:year,m:month},function(data){
	        if(data != null){
	...
	$('#ncsubmit').click(function(){
	    $("#glist").flexOptions({url: 'index.php?model=stat_marketing&fun=get_grouplist_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
	    $("#flexigrid").flexOptions({url: 'index.php?model=stat_marketing&fun=get_groupgoods_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
	    update_flex();
    });
});
function fg_operation(name, bDiv){
    var stat_url = 'index.php?model=stat_marketing&fun=groupgoods';
