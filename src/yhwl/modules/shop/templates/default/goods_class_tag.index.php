...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?model=goods_class&fun=get_xml',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'update') {
        window.location.href = 'index.php?model=goods_class&fun=tag_update';
    }
    if (name == 'reset') {
        window.location.href = 'index.php?model=goods_class&fun=tag_reset';
    }
}
function fg_edit(id) {
    CUR_DIALOG = ajax_form('tag_edit', '编辑TAG', 'index.php?model=goods_class&fun=tag_edit&id='+id, 640);
}
</script>