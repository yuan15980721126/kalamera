...
class storeControl extends SystemControl{
    ...
    public function edit_save_joininOp() {
        if (chksubmit()) {
            ...
            if ($result) {
                showMessage(L('nc_common_op_succ'), 'index.php?model=store&fun=store');
            } else {
    ...
    public function get_joinin_xmlOp() {
        ...
        $param = array('member_id', 'member_name', 'sg_id', 'paying_amount', 'joinin_state', 'joinin_year', 'contacts_name', 'contacts_phone'
                ,'contacts_email', 'company_name', 'company_province_id', 'company_phone', 'company_employee_count', 'company_registered_capital','jiameng_time'
        );
        if (in_array($_POST['sortname'], $param) && in_array($_POST['sortorder'], array('asc', 'desc'))) {
            $order = $_POST['sortname'] . ' ' . $_POST['sortorder'];
        }
        ...
        foreach ($store_list as $value) {
            ...
            if(in_array(intval($value['joinin_state']), array(STORE_JOIN_STATE_NEW, STORE_JOIN_STATE_PAY))) {
                $operation = "<a class='btn orange' href=\"index.php?model=store&fun=store_joinin_detail&id=". $value['id'] ."\"><i class=\"fa fa-check-circle\"></i>审核</a>";
            } else {
                $operation = "<a class='btn green' href=\"index.php?model=store&fun=store_joinin_detail&id=". $value['id'] ."\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
            $param['operation'] = $operation;
            ...
            $param['jiameng_message'] = $value['jiameng_message'];
            $param['jiameng_time'] = date('Y-m-d H:i:s',$value['jiameng_time']);
    ...
    public function store_joinin_verifyOp() {
        $model_store_joinin = Model('store_joinin');
        $joinin_detail = $model_store_joinin->getOne(array('id'=>$_POST['id']));
        // print_R($data);
    ...
    private function store_joinin_verify_pass($joinin_detail) {
        ...
        $model_store_joinin = Model('store_joinin');
        $model_store_joinin->modify($param, array('id'=>$_POST['id']));
        // echo "<pre>";
        // print_R($param);die;
        if ($param['paying_amount'] > 0) {
            // showMessage('店铺入驻申请审核完成','index.php?model=store&fun=store_joinin');
            showMessage('留言申请审核完成','index.php?model=store&fun=store_joinin');
        } else {
