...
class contractControl extends SystemControl{
    ...
    public function __construct(){
        ...
        if (C('contract_allow') != 1){
            showDialog('需开启“消费者保障服务”功能','index.php?model=operating','error');
        }
    ...
    public function citemeditOp(){
        ...
        if ($itemid_id <= 0){
            showDialog(L('param_error'),'index.php?model=contract&fun=citemlist');
        }
        $model_contract = Model('contract');
        //查询模板信息
        $where = array();
        ...
        if(chksubmit()){
            ...
            if($rs){
                ...
                showDialog(L('nc_common_save_succ'),'index.php?model=contract&fun=citemlist','succ');
            } else {
    ...
    public function contracteditOp(){
        ...
        if ($ct_id <= 0){
            showDialog(L('param_error'),'index.php?model=contract&fun=contractlist');
        }
        $model_contract = Model('contract');
        //查询店铺保障服务信息
        $where = array();
        ...
        if(chksubmit()){
            ...
            if ($c_info['ct_closestate_key'] == $_POST['c_state']) {
                showDialog(L('nc_common_save_succ'),'index.php?model=contract&fun=contractlist','succ');
            }
            try {
    ...
    public function applyeditOp(){
        ...
        if ($aid <= 0){
            showDialog(L('param_error'),'index.php?model=contract&fun=applylist');
        }
        $model_contract = Model('contract');
        //查询申请信息
        $where = array();
        ...
        if(chksubmit()){
            if ($_POST['apply_state'] == $apply_info['cta_auditstate_key']) {
                showDialog('不能重复更改申请状态','index.php?model=contract&fun=applylist');
            }
            if (!in_array($_POST['apply_state'],array_keys($curr_applystatearr))) {
                showDialog('编辑失败','index.php?model=contract&fun=applyedit&aid='.$apply_info['cta_id']);
            }
            try {
                $model_contract->beginTransaction();
                //编辑申请状态
                ...
                showDialog(L('nc_common_save_succ'),'index.php?model=contract&fun=applylist','succ');
            }catch (Exception $e){
                $model_contract->rollback();
                showDialog($e->getMessage(), '', 'error');
            }
    ...
    public function quiteditOp(){
        ...
        if ($qid <= 0){
            showDialog(L('param_error'),'index.php?model=contract&fun=quitlist');
        }
        $model_contract = Model('contract');
        //查询申请信息
        $where = array();
        ...
        if(chksubmit()){
            if ($_POST['quit_state'] == $quit_info['ctq_auditstate_key']) {
                showDialog('不能重复编辑申请状态','index.php?model=contract&fun=quitlist');
            }
            if (!in_array($_POST['quit_state'],array_keys($this->quit_auditstate_arr))) {
                showDialog('编辑失败','index.php?model=contract&fun=quitedit&qid='.$quit_info['ctq_id']);
            }
            try {
                $model_contract->beginTransaction();
                //编辑申请状态
                ...
                showDialog(L('nc_common_save_succ'),'index.php?model=contract&fun=quitlist','succ');
            }catch (Exception $e){
                $model_contract->rollback();
                showDialog($e->getMessage(), '', 'error');
            }
    ...
    private function show_menu($menu_key=''){
        $menu_array = array(
            1=>array('menu_key'=>'applylist','menu_name'=>'服务加入申请', 'menu_url'=>'index.php?model=contract&fun=applylist'),
            2=>array('menu_key'=>'quitlist','menu_name'=>'服务退出申请', 'menu_url'=>'index.php?model=contract&fun=quitlist'),
            3=>array('menu_key'=>'contractlist','menu_name'=>'店铺保障服务', 'menu_url'=>'index.php?model=contract&fun=contractlist'),
            4=>array('menu_key'=>'citemlist','menu_name'=>'保障服务管理', 'menu_url'=>'index.php?model=contract&fun=citemlist')
        );
