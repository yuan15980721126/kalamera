...
class repaidControl extends SystemControl{
    const EXPORT_SIZE = 1000;

    private $_links = array(
        array('url'=>'model=store&fun=store','text'=>'管理'),
        array('url'=>'model=store&fun=store_joinin','text'=>'开店申请'),
        array('url'=>'model=store&fun=reopen_list','text'=>'续签申请'),
        array('url'=>'model=store&fun=store_bind_class_applay_list','text'=>'经营类目申请'),
        array('url'=>'model=store&fun=bill_cycle','text'=>'结算周期设置')
    );
    ...
    public function get_joinin_xmlOp() {
        ...
        // $condition['state'] = 1;
        $order = 'add_time desc';
        $param = array('member_id', 'member_name', 'goods_name', 'goods_class', 'buy_time', 'order_sn', 'desc', 'contacts_phone','contacts_name'
                ,'contacts_email', 'member_areaid', 'member_cityid', 'member_provinceid', 'member_areainfo', 'address','add_time','state'
        );
        ...
        $model_good = Model('goods_class');
        $attribute = Model('attribute');
        foreach ($store_list as $value) {
            $param = array();
            if(in_array(intval($value['state']), array(1))) {
                $operation = "<a class='btn orange' href=\"index.php?model=repaid&fun=store_joinin_detail&id=". $value['id'] ."\"><i class=\"fa fa-check-circle\"></i>审核</a>";
            } else {
                $operation = "<a class='btn green' href=\"index.php?model=repaid&fun=store_joinin_detail&id=". $value['id'] ."\"><i class=\"fa fa-list-alt\"></i>查看</a><a class='btn red' href=\"javascript:void(0);\" onclick=\"fg_cancel({$value['id']})\">删除</a>删除</a>";
            }
            ...
            $condition = array();
            $condition['gc_id'] = $value['goods_name'];
            $_name = $model_good->getGoodsClassList($condition,'gc_name');
            $param['goods_name'] = $_name[0]['gc_name'];

            $condition = array();

            $condition['attr_id'] = $value['goods_class'];
            $_val = $attribute->getAttributeList($condition,'attr_value');
            $param['goods_class'] = $_val[0]['attr_value'];

            // print_R($_val);
            // $param['goods_class'] = $value['goods_class'];
            $param['order_sn'] = $value['order_sn'];
            $param['description'] = $value['description'];
            $param['order_sn'] = $value['order_sn'];
            $param['order_sn'] = $value['order_sn'];
            // $param['joinin_state'] = $joinin_state_array[$value['joinin_state']];
            // $param['joinin_year'] = $value['xm'];
            ...
            $param['jiameng_message'] = $value['member_areainfo'];
    ...
    public function deleteOp() {
        if (preg_match('/^[\d,]+$/', $_GET['id'])) {
            ...
            $repaid = Model('repaid');
            // print_R($_GET['id']);die;
            $result = $repaid->drop(array('id' => array('in', $_GET['id'])));
            if ($result) {
                $this->log('删除商品报修记录，评价编[ID:'.implode(',',$_GET['id']).']',null);
    ...
    public function storeOp(){
        ...
        // Tpl::output('top_link',$this->sublink(array($this->_links['1']),'store'));
        // echo "<pre>";
        // print_R($this->sublink(array($this->_links['1']),'store'));
        // die;
		Tpl::setDirquna('shop');
    ...
    public function get_xmlOp() {
        ...
        foreach ($store_list as $value) {
            ...
            $operation = "<a class='btn green' href='index.php?model=store&fun=store_joinin_detail&member_id=".$value['member_id']."'><i class='fa fa-list-alt'></i>查看</a><span class='btn'><em><i class='fa fa-cog'></i>" . L('nc_set') . " <i class='arrow'></i></em><ul><li><a href='index.php?model=store&fun=store_edit&store_id=" . $value['store_id'] . "'>编辑店铺信息</a></li><li><a href='index.php?model=store&fun=store_bind_class&store_id=" . $value['store_id'] . "'>修改经营类目</a></li>";
    ...
    public function get_bill_cycle_xmlOp() {
        ...
        foreach ($store_list as $value) {
            $param = array();
            $store_state = $this->getStoreState($value);
            $operation = "<a class='btn blue' href='index.php?model=store&fun=bill_cycyle_edit&store_id=".$value['store_id']."'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            $operation .= "</ul></span>";
            $param['operation'] = $operation;
            $param['store_id'] = $value['store_id'];
            $store_name = "<a class='" . $store_state . "' href='". urlShop('show_store', 'index', array('store_id' => $value['store_id'])) ."' target='blank'>";
    ...
    public function export_csvOp() {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?model=store&fun=index');
				Tpl::setDirquna('shop');
    ...
    public function store_editOp(){
        ...
        if (chksubmit()){
            ...
            if ($result){
                $url = array(
                array(
                'url'=>'index.php?model=store&fun=store',
                'msg'=>$lang['back_store_list'],
                ),
                array(
                'url'=>'index.php?model=store&fun=store_edit&store_id='.intval($_POST['store_id']),
    ...
    public function bill_cycyle_editOp(){
        ...
        if (chksubmit()){
            ...
            if ($result){
                ...
                showMessage($lang['nc_common_save_succ'],'index.php?model=store&fun=bill_cycle');
            }else {
                $this->log('设置店铺结算周期['.$_POST['store_name'].']',1);
                showMessage($lang['nc_common_save_fail'],'index.php?model=store&fun=bill_cycle');
            }
        }
    ...
    public function edit_save_joininOp() {
        if (chksubmit()) {
            ...
            if ($result) {
                showMessage(L('nc_common_op_succ'), 'index.php?=store&fun=store');
            } else {
    ...
    public function haoshop_addOp()
    {
        if (chksubmit())
        {
            ...
            showMessage('操作成功','index.php?model=store&fun=store');
            return;
        }
		Tpl::setDirquna('shop');
    ...
    public function get_bind_class_applay_xmlOp() {
        ...
        foreach ($store_bind_class_list as $value) {
            $param = array();
            if($value['state'] == '0') {
                $operation = "<a class='btn orange' href=\"javascript:if(confirm('确认审核吗？'))window.location = 'index.php?model=store&fun=store_bind_class_applay_check&bid=".$value['bid']."&store_id=".$value['store_id']."'\"><i class=\"fa fa-check-circle\"></i>审核</a>";
            } else {
                $operation = "<a class='btn red' href=\"javascript:if(confirm('".($value['state'] == '1' ? '该类目已经审核通过，删除它可能影响到商家的使用，' : null)."确认删除吗？'))window.location = 'index.php?model=store&fun=store_bind_class_applay_del&bid=".$value['bid']."&store_id=".$value['store_id']."'\"><i class=\"fa fa-trash-o\"></i>删除</a>";
            }
            $param['operation'] = $operation;
            $param['store_id'] = $value['store_id'];
    ...
    public function store_joinin_detailOp(){
        ...
        // print_R($joinin_detail);
        $joinin_detail_title = '查看';
        if(in_array(intval($joinin_detail['state']), array(1))) {
            $joinin_detail_title = '审核';
        }
        $model_good = Model('goods_class');
        $attribute = Model('attribute');
        $condition = array();
        $condition['gc_id'] = $joinin_detail['goods_name'];
        $_name = $model_good->getGoodsClassList($condition,'gc_name');
        $joinin_detail['goods_name'] = $_name[0]['gc_name'];


        $condition = array();

        $condition['attr_id'] = $joinin_detail['goods_class'];
        $_val = $attribute->getAttributeList($condition,'attr_value');
        $joinin_detail['goods_class'] = $_val[0]['attr_value'];
        $joinin_detail['add_time'] = date('Y-m-d', $joinin_detail['add_time']);
    ...
    public function store_joinin_verifyOp() {
        $model_store_joinin = Model('repaid');
        $joinin_detail = $model_store_joinin->getOne(array('member_id'=>$_POST['member_id'],'id'=>$_POST['cid']));
        // echo "<pre>";
        // print_R($joinin_detail);die;
        switch (intval($joinin_detail['state'])) {
            case 1:
    ...
    private function store_joinin_verify_pass($joinin_detail) {
        $param = array();
        $param['state'] = $_POST['verify_type'] === 'pass' ? 2 : 3;
        ...
        $model_store_joinin = Model('repaid');
        $update = $model_store_joinin->modify($param, array('member_id'=>$_POST['member_id'],'id'=>$_POST['cid']));
        // echo "<pre>";
        // print_R($param);die;
        if ($update) {
            showMessage('商品报修申请审核完成','index.php?model=repaid&fun=store_joinin');
        } 
        // else {
    ...
    private function store_joinin_verify_open($joinin_detail) {
        ...
        if($_POST['verify_type'] === 'pass') {
            ...
            if($state) {
                ...
                showMessage('店铺开店成功','index.php?model=store&fun=store_joinin');
            } else {
                showMessage('店铺开店失败','index.php?model=store&fun=store_joinin');
            }
        } else {
            showMessage('店铺开店拒绝','index.php?model=store&fun=store_joinin');
        }
    }
