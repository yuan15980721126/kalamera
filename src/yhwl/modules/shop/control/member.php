...
class memberControl extends SystemControl{
    ...
    public function member_editOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if (!empty($_POST['member_exppoints'])){
                    $update_array['member_exppoints']   = intval($_POST['member_exppoints']);//新增经验值修改功能
                }
                ...
                if ($result){
                    if(!empty($update_array['member_exppoints'])){
                        $this->log('修改id为'.$update_array['member_id'].'的会员经验值成'.$update_array['member_exppoints'],1);
                    }
                    $url = array(
                    array(
    ...
    private function createCsv($member_list) {
        ...
        foreach ($member_list as $value) {
            ...
            $param['member_truename'] = $value['member_truename'];
            $param['member_birthday'] = $value['member_birthday'];
            $param['member_time'] = $value['member_time'] ? date('Y-m-d', $value['member_time']) : '';
            $param['member_login_time'] = $value['member_login_time'] ? date('Y-m-d', $value['member_login_time']) : '';
            $param['member_login_ip'] = $value['member_login_ip'];
            $param['member_points'] = $value['member_points'];
            $param['member_exppoints'] = $value['member_exppoints'];
            $param['member_grade'] = ($t = $model_member->getOneMemberGrade($value['member_exppoints'], false, $member_grade))?$t['level_name']:'';
       ...
       array_unshift($data, $header);
		$csv = new Csv();
	    $export_data = $csv->charset($data,CHARSET);
