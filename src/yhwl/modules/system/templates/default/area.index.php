...
var parent_id='<?php echo intval($_GET['parent_id']);?>'
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?model=area&fun=get_xml&parent_id='+parent_id,
        colModel : [
            ...
            {display: '地区税额', name : 'tax', width : 100, sortable : false, align : 'left'},
...
function fg_operate(name, grid) {
    if (name == 'add') {
        window.location.href = 'index.php?model=area&fun=add&parent_id='+cur_parent_id;
    }else if (name == 'delete') {
...
function fg_delete(id) {
	...
	$.ajax({
        type: "GET",
        dataType: "json",
        url: "index.php?model=area&fun=del",
        data: "area_id="+id,
...
function fg_show_children(area_id,parent_id) {
	$("#flexigrid").flexOptions({url: 'index.php?model=area&fun=get_xml&parent_id='+area_id}).flexReload();
...
function fg_up() {
	...
	$("#flexigrid").flexOptions({url: 'index.php?model=area&fun=get_xml&parent_id='+his_parent_ids[his_parent_ids.length-1]}).flexReload();
