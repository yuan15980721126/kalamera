...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?model=member&fun=get_xml',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?model=member&fun=member_add';
    }
    if (name == 'csv') {
...
function submit_delete(id){
	...
	if(confirm('删除后将不能恢复，确认删除这 ' + id.length + ' 项吗？')){
		id = id.join(',');
        window.location.href = 'index.php?model=member&fun=member_del&member_id='+id;
    }
}
