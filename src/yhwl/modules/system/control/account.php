...
class accountControl extends SystemControl{
    private $links = array(
    //    array('url'=>'model=account&fun=qq','lang'=>'qqSettings'),
    //    array('url'=>'model=account&fun=sina','lang'=>'sinaSettings'),
    //    array('url'=>'model=account&fun=sms','text'=>'手机短信'),
    //    array('url'=>'model=account&fun=wx','text'=>'微信登录'),
		//array('url'=>'model=account&fun=uc','text'=>'UC互联')
        array('url'=>'model=account&fun=facebook','text'=>'Facebook登录'),
        /*
        array('url'=>'model=account&fun=google','text'=>'Google登录'),
        array('url'=>'model=account&fun=twitter','text'=>'Twitter登录'),
        */
    );
     ...
     * Facebook登录设置
     */
    public function facebookOp(){
        $model_setting = Model('setting');
        if (chksubmit()){
            $update_array = array();
            $update_array['facebook_isuse']   = $_POST['facebook_isuse'];
            $update_array['facebook_appid']   = $_POST['facebook_appid'];
            $update_array['facebook_secret']  = $_POST['facebook_secret'];
            $result = $model_setting->updateSetting($update_array);
            if ($result){
                $this->log('编辑账号同步，Facebook登录设置');
                showMessage(Language::get('nc_common_save_succ'));
            }else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        ...
        Tpl::output('list_setting',$list_setting);
        //输出子菜单
        Tpl::output('top_link',$this->sublink($this->links,'facebook'));
        Tpl::setDirquna('system');
        Tpl::showpage('account.facebook_setting');
    }
