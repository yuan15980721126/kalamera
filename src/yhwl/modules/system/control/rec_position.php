...
class rec_positionControl extends SystemControl{
    ...
    public function get_xmlOp(){
        ...
        foreach ($list as $k => $info) {
            ...
            $operation_detail = "<li><a href='index.php?model=rec_position&fun=rec_edit&rec_id={$info['rec_id']}'>编辑内容</a></li>";
            $operation_detail .= "<li><a href=\"javascript:void(0);\" rec_id=\"{$info['rec_id']}\" nctype=\"jscode\"></i>调用代码</a></li>";
            $operation_detail .= "<li><a href=\"index.php?model=rec_position&fun=rec_view&rec_id={$info['rec_id']}\" target=\"_blank\">预览效果</a></li>";
            if ($operation_detail) {
    ...
    public function rec_saveOp(){
        ...
        showMessage(Language::get('nc_common_save_succ'),'index.php?model=rec_position&fun=rec_list');
    }
...
//                        $array['body'][$k]['url']   = preg_match($pattern,$_POST['urlup'][$k]) ? $_POST['urlup'][$k] : '';
                        $array['body'][$k]['url']   = $_POST['urlup'][$k];
                    }
                    $ext = strtolower(pathinfo($_FILES['pic']['name'][$k], PATHINFO_EXTENSION));
                    if (in_array($ext,array('jpg','jpeg','gif','png','bmp'))){
                        $filename = substr(md5(microtime(true)),0,16).rand(100,999).$k.'.'.$ext;
        ...
        if ($result){
            ...
            showMessage(Language::get('nc_common_save_fail'),'index.php?model=rec_position&fun=rec_list');
        }
    }
