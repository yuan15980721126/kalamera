...
class member_bindControl extends BaseMemberControl {
    ...
    private function profile_menu($menu_key='',$array=array()) {
        ...
        $menu_array     = array();
        $menu_array = array(
            1=>array('menu_key'=>'qq_bind', 'menu_name'=>'QQ绑定',   'menu_url'=>'index.php?model=member_bind&fun=qqbind'),
            2=>array('menu_key'=>'sina_bind',   'menu_name'=>'新浪绑定', 'menu_url'=>'index.php?model=member_bind&fun=sinabind'),
            3=>array('menu_key'=>'weixin_bind',   'menu_name'=>'微信绑定', 'menu_url'=>'index.php?model=member_bind&fun=weixinbind'),
        );
        if(!empty($array)) {
            $menu_array[] = $array;
        }
