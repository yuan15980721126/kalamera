...
<form method="GET">
  <input type="hidden" name="model" value="store"/>
  <input type="hidden" name="fun" value="activity_apply"/>
  ...
  <table class="ncsc-default-table" >
    ...
    <tbody>
      ...
      <tr class="bd-line">
        <td><div class="pic-thumb"><a href="index.php?model=goods&goods_id=<?php echo $v['goods_id']; ?>" target="_blank"><img src="<?php echo cthumb($v['goods_image'], 60,$_SESSION['store_id']);?>"></a></div></td>
  ...
  <div class="div-goods-select">
    <form method="GET">
      <input type="hidden" name="model" value="store_activity"/>
      <input type="hidden" name="fun" value="activity_apply"/>
      <input type="hidden" name="activity_id" value="<?php echo $_GET['activity_id'];?>"/>
      <table class="search-form">
        <tr>
          <th class="w250"><strong>选择参加活动的商品，勾选并提交平台审核</strong></th>
          <td class="w160"><input type="text" class="text w150" name="name" value="<?php echo $output['search']['name'];?>" placeholder="search商品名称"/></td>
    ...
    <form method="POST" id="apply_form" onsubmit="ajaxpost('apply_form','','','onerror');" action="index.php?model=store_activity&fun=activity_apply_save">
      <input type="hidden" name="activity_id" value="<?php echo $_GET['activity_id'];?>"/>
