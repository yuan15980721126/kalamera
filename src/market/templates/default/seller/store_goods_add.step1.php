...
<div class="wrapper_search">
  ...
  <div class="wp_confirm">
    <form method="get">
      ...
      <input type="hidden" name="fun" value="edit_goods" />
      <input type="hidden" name="commonid" value="<?php echo $output['commonid'];?>" />
      <input type="hidden" name="ref_url" value="<?php echo $_GET['ref_url'];?>" />
      <?php } else {?>
      <input type="hidden" name="model" value="store_goods_add" />
      <input type="hidden" name="fun" value="add_step_two" />
      <?php }?>
...
$(function(){
      showDialog('找不到商品分类，点击确定去申请经营类目，审核通过后再来发布商品。','confirm', '', function(){
		  window.location.href='index.php?model=store_info&fun=bind_class';
		});
})
      <?php }?>
</script>
