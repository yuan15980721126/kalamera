  ...
  <form method="post" action="<?php echo empty($output['chain_info']) ? urlShop('store_chain', 'chain_add') : urlShop('store_chain', 'chain_edit', array('chain_id' => $output['chain_info']['chain_id']));?>" id="chain_form" enctype="multipart/form-data">
    ...
    <dl>
      ...
      <dd>
        ...
        <p class="hint">登录名请使用中文、字母、数字、下划线（最低三个字符），Registered successfully后不可以修改。</p>
      </dd>
    </dl>
    <dl>
      <dt>
    ...
    $('#chain_form').validate({
        ...
        rules : {
            chain_user : {
                required : true,
                minlength: 3,
                remote   : 'index.php?model=store_chain&fun=check_user<?php if (!empty($output['chain_info'])) {?>&no_id=<?php echo $output['chain_info']['chain_id'];}?>'
            },
            chain_pwd : {
                <?php if (empty($output['chain_info'])) {?>
                required : true,
