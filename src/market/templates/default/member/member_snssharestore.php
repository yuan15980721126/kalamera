...
<div class="dialog-share">
  <form method="post" action="index.php?model=member_snsindex&fun=sharestore&<?php if($_GET['irefresh'] == 1){?>irefresh=1<?php }?>" id="sharestore_form">
...
$(function(){
	...
	$("#weibobtn_store").click(function(){
		if($("#sharestore_form").valid()){
			...
			if(cookienum >= max_recordnum && $("#ss_seccode").css('display') == 'none'){
				...
				$("#ss_seccode").find("[name='codeimage']").attr('src','index.php?model=seccode&fun=makecode&nchash='+nchash+'&t=' + Math.random());
	...
	$("[nc_type='bindbtn']").bind('click',function(){
	    ...
	    if(isbind == '1'){//已经绑定
		    ...
		    window.open(MEMBER_SITE_URL + '/api.php?model=sharebind&type='+data_str.apikey);
		}
	});
