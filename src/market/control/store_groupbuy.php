...
class store_groupbuyControl extends BaseSellerControl {
    ...
    public function __construct() {
        ...
        if (intval(C('groupbuy_allow')) !== 1){
            showMessage(Language::get('groupbuy_unavailable'),'index.php?model=seller_center','','error');
        }
    }
    /**
     * 默认显示抢购列表
    ...
    public function groupbuy_addOp() {
        ...
        if (checkPlatformStore()) {
            ...
            $current_groupbuy_quota = $model_groupbuy_quota->getGroupbuyQuotaCurrent($_SESSION['store_id']);
            if (empty($current_groupbuy_quota)) {
                showMessage('No current package available, please buy the package first', urlShop('store_groupbuy', 'groupbuy_quota_add'), '', 'error');
                //当前没有可用套餐，请先购买套餐
                Tpl::output('current_groupbuy_quota', $current_groupbuy_quota);
            }
        }
    ...
    public function groupbuy_saveOp() {
        ...
        if ($_GET['vr']) {
            if ($param['upper_limit'] > 0 && $goods_info['virtual_limit'] > 0
                ...
                showDialog(sprintf('虚拟抢购活动的限购数量(%d)不能大于虚拟商品本身的限购数量(%d)',
                    $param['upper_limit'], $goods_info['virtual_limit']
                    ), 'index.php?model=store_groupbuy');
            }
        ...
        if($result) {
            ...
            showDialog(Language::get('groupbuy_add_success'),'index.php?model=store_groupbuy','succ');
        }else {
            showDialog(Language::get('groupbuy_add_fail'),'index.php?model=store_groupbuy');
        }
    }
    ...
    private function profile_menu($menu_key='') {
        ...
        switch ($menu_key){
        case 'groupbuy_add':
            $menu_array[] = array('menu_key'=>'groupbuy_add','menu_name'=>L('nc_member_path_new_group'),'menu_url'=>'index.php?model=store_groupbuy&fun=groupbuy_add');
            break;
        case 'groupbuy_add_vr':
            $menu_array[] = array('menu_key'=>'groupbuy_add_vr','menu_name'=>'新增虚拟抢购','menu_url'=>'index.php?model=store_groupbuy&fun=groupbuy_add_vr');
            break;
        case 'groupbuy_quota_add':
            ...
            break;
        case 'groupbuy_edit':
            $menu_array[] = array('menu_key'=>'groupbuy_edit','menu_name'=>L('nc_member_path_edit_group'),'menu_url'=>'index.php?model=store_groupbuy');
            break;
        case 'cancel':
