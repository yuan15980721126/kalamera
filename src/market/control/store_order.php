...
class store_orderControl extends BaseSellerControl {
    ...
    public function show_orderOp() {
        ...
        foreach ($order_info['extend_order_goods'] as $value) {
            ...
            $value['goods_repair'] = unserialize($value['goods_repair']);
...
//        print_r($order_info);die;
        $new_address = $this->get_area($order_info['extend_order_common']['area_code'],$order_info['extend_order_common']['reciver_city_id'],$order_info['extend_order_common']['area_name'],$order_info['extend_order_common']['reciver_info']['street']);
        Tpl::output('new_address',$new_address);
    ...
    public function change_stateOp() {
        ...
        if (!$result['state']) {
            ...
            if ($_GET['state_type'] == 'spay_price') {//后台修改订单价格时 重新修改支付单号pay_sn
                $pay_sn_new = Logic('buy_1')->makePaySn($order_info['buyer_id']);

                $pay_info = $model_order->getOrderPayInfo(array('pay_sn' => $order_info['pay_sn'], 'buyer_id' => $order_info['buyer_id']), true);

                $update_pay = $model_order->editOrderPay(array('pay_sn' => $pay_sn_new, 'api_pay_state' => 0), array('pay_id' => $pay_info['pay_id']));
                $update = $model_order->editOrder(array('pay_sn' => $pay_sn_new, 'pay_sn1' => $order_info['pay_sn']), array('order_id' => $order_id, 'order_type' => $order_info['order_type']));
    ...
    private function profile_menu($menu_type='',$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
            $menu_array = array(
            array('menu_key'=>'store_order',        'menu_name'=>Language::get('nc_member_path_all_order'), 'menu_url'=>'index.php?model=store_order'),
            array('menu_key'=>'state_new',          'menu_name'=>Language::get('nc_member_path_wait_pay'),  'menu_url'=>'index.php?model=store_order&fun=index&state_type=state_new'),
            array('menu_key'=>'state_pay',          'menu_name'=>Language::get('nc_member_path_wait_send'), 'menu_url'=>'index.php?model=store_order&fun=store_order&state_type=state_pay'),
            array('menu_key'=>'state_notakes',      'menu_name'=>'待自提', 'menu_url'=>'index.php?model=store_order&fun=store_order&state_type=state_notakes'),
            array('menu_key'=>'state_send',         'menu_name'=>Language::get('nc_member_path_sent'),      'menu_url'=>'index.php?model=store_order&fun=index&state_type=state_send'),
            array('menu_key'=>'state_success',      'menu_name'=>Language::get('nc_member_path_finished'),  'menu_url'=>'index.php?model=store_order&fun=index&state_type=state_success'),
            array('menu_key'=>'state_cancel',       'menu_name'=>Language::get('nc_member_path_canceled'),  'menu_url'=>'index.php?model=store_order&fun=index&state_type=state_cancel'),
            );
            break;
        }
    ...
    public function get_area($area_code,$city_id,$area_name,$street){
        $model_area = Model('area');
        $area_info = $model_area->getAreaInfo(array('area_code'=>$area_code));
        $city_info = $model_area->getAreaInfo(array('area_id'=>$city_id));


        $new = $street.' '.$area_name.', '.$city_info['area_name'].', '.$area_info['area_name'];
//        echo '<pre>';
//        print_r($new);die;
        return $new;
