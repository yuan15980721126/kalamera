...
class Control{
     ...
     *
     */
    protected function showFooterLayout() {
        //首页底部公告信息
        $model_article = Model('article');
        $condition['ac_ids']	= '13';
        $condition['limit']	= '2';
        $bottom_list = $model_article->getArticleList($condition,$page);

        $condition['ac_ids']	= '15,16,17,18,19';
        $condition['limit']	= '5';
        $condition['order']	=  'article_sort asc';
        $option_list = $model_article->getArticleList($condition,$page);
        ...
        $condition['pic_type'] = 1;

        $position_list = $model_rec->where($condition)->select();
//        echo "<pre>";
//        print_R($option_list);
        foreach ($position_list as $key => $val){
            $oinfo = unserialize($val['content']);
            $position_list[$key]['pic'] = UPLOAD_SITE_URL.'/'.$oinfo['body'][0]['title'];
            $position_list[$key]['url'] = $oinfo['body'][0]['url'];

        }
        //print_R($list);

        Tpl::output('position_list',$position_list);
        Tpl::output('bottom_list',$bottom_list);
...
// print_R($goods_class);
    ...
    protected function getMemberAndGradeInfo($is_return = false){
        ...
        $list_setting['member_grade'] = $list_setting['member_grade']?unserialize($list_setting['member_grade']):array();
         // echo "<pre>";
         // print_R($list_setting['member_grade']);
        //会员详情及会员级别处理
        if($_SESSION['member_id']) {
        ...
        foreach ($list_setting['member_grade'] as $key => $val) {//获取会员折扣
            if($val['level'] == $member_info['level']){
                if(!empty($val['orderdiscount'])){
                    $discount = $val['orderdiscount'];
                    break; 
                }
            }
        }
        $member_info['orderdiscount'] = $discount;
        // print_R($member_info);
    ...
    protected function checkLogin(){
        if ($_SESSION['is_login'] !== '1'){
            if (trim($_GET['fun']) == 'favoritegoods' || trim($_GET['fun']) == 'favoritestore'){
    ...
    protected function article() {
        ...
        $code_array = array('member','payment','sold','service','mainright');
        $notice_limit   = 5;
        $faq_limit  = 5;
    ...
    private function _getMenuList() {
        ...
        $menu_list = array(
            ...
            'client' => array('name' => '客户服务', 'child' => array(
                ...
                // 'member_repair'     => array('name' => '商品报修', 'url' => urlShop('member_repair','index')),
...
class BaseSNSControl extends Control {
    ...
    public function get_member_info() {
        ...
        if(empty($member_info)){
            showMessage(L('wrong_argument'), 'index.php?model=member_snshome', '', 'error');
        }
        //粉丝数
...
class BaseSellerControl extends Control {
    ...
    public function __construct(){
        ...
        Tpl::output('nav_list', rkcache('nav',true));
        if ($_GET['model'] !== 'seller_login') {

            if(empty($_SESSION['seller_id'])) {
                @header('location: index.php?model=seller_login&fun=show_login');die;
            }
            ...
            if ($_SESSION['seller_is_admin'] !== 1 && $_GET['model'] !== 'seller_center' && $_GET['model'] !== 'seller_logout') {
                if (!in_array($_GET['model'], $_SESSION['seller_limits'])) {
    ...
    protected function recordSellerLog($content = '', $state = 1){
        ...
        $seller_info['log_url'] = $_GET['model'].'&'.$_GET['fun'];
        ...
        if (intval($is_admin) !== 1) {
            ...
            foreach ($menu_list as $key => $value) {
                foreach ($value['child'] as $child_key => $child_value) {
                    if (!in_array($child_value['model'], $limits)) {
...
//        die;
    ...
    private function _getCurrentMenu($seller_function_list) {
        $current_menu = $seller_function_list[$_GET['model']];
        if(empty($current_menu)) {
            $current_menu = array(
                'model' => 'index',
                'model_name' => 'home page'
            );
        }
        return $current_menu;
    }
    ...
    private function _getMenuList() {
        $menu_list = array(
            'goods' => array('name' => '商品', 'child' => array(
                array('name' => '商品发布', 'model'=>'store_goods_add', 'fun'=>'index'),
// 				array('name' => '淘宝CSV导入', 'model'=>'taobao_import', 'fun'=>'index'),
                array('name' => '出售中的商品', 'model'=>'store_goods_online', 'fun'=>'index'),
                array('name' => '仓库中的商品', 'model'=>'store_goods_offline', 'fun'=>'index'),
//                 array('name' => '预约/到货通知', 'act' => 'store_appoint', 'op' => 'index'),
                array('name' => '关联版式', 'model'=>'store_plate', 'fun'=>'index'),
                array('name' => '商品规格', 'model' => 'store_spec', 'op' => 'index'),
                array('name' => '图片空间', 'model'=>'store_album', 'fun'=>'album_cate'),
            )),
            'order' => array('name' => '订单物流', 'child' => array(
                array('name' => '交易订单', 'model'=>'store_order', 'fun'=>'index'),
//                 array('name' => '虚拟兑码订单', 'model'=>'store_vr_order', 'fun'=>'index'),
                array('name' => '发货', 'model'=>'store_deliver', 'fun'=>'index'),
                array('name' => '发货设置', 'model'=>'store_deliver_set', 'fun'=>'daddress_list'),
                array('name' => '运单模板', 'model'=>'store_waybill', 'fun'=>'waybill_manage'),
                array('name' => '评价管理', 'model'=>'store_evaluate', 'fun'=>'list'),
                array('name' => '物流工具', 'model'=>'store_transport', 'fun'=>'index'),
				array('name' => '来单提醒', 'model'=>'order_call', 'fun'=>'index'),
            )),
            'promotion' => array('name' => '促销', 'child' => array(
                array('name' => '抢购管理', 'model'=>'store_groupbuy', 'fun'=>'index'),
//                 array('name' => '加价购', 'model'=>'store_promotion_cou', 'fun'=>'cou_list'),
                array('name' => '限时折扣', 'model'=>'store_promotion_xianshi', 'fun'=>'xianshi_list'),
                array('name' => '满即送', 'model'=>'store_promotion_mansong', 'fun'=>'mansong_list'),
//                 array('name' => '优惠套装', 'model'=>'store_promotion_bundling', 'fun'=>'bundling_list'),
...
//                 array('name' => '手机专享', 'act' => 'store_promotion_sole', 'op' => 'index'),
                array('name' => '代金券管理', 'model'=>'store_voucher', 'fun'=>'templatelist'),
                array('name' => '活动管理', 'model'=>'store_activity', 'fun'=>'store_activity'),
            )),
//             'store' => array('name' => '店铺', 'child' => array(
//                 array('name' => '店铺设置', 'model'=>'store_setting', 'fun'=>'store_setting'),
//                 array('name' => '店铺装修', 'model'=>'store_decoration', 'fun'=>'decoration_setting'),
//                 array('name' => '店铺导航', 'model'=>'store_navigation', 'fun'=>'navigation_list'),
//                 array('name' => '店铺动态', 'model'=>'store_sns', 'fun'=>'index'),
//                 array('name' => '店铺信息', 'model'=>'store_info', 'fun'=>'bind_class'),
//                 array('name' => '店铺分类', 'model'=>'store_goods_class', 'fun'=>'index'),
//                 array('name' => '品牌申请', 'model'=>'store_brand', 'fun'=>'brand_list'),
//                 array('name' => '供货商', 'model'=>'store_supplier', 'fun'=>'sup_list'),
//                 array('name' => '实体店铺', 'model'=>'store_map', 'fun'=>'index'),
//                 array('name' => '消费者保障服务', 'model'=>'store_contract', 'fun'=>'index'),
//             )),
            'consult' => array('name' => '售后服务', 'child' => array(
                array('name' => '咨询管理', 'model'=>'store_consult', 'fun'=>'consult_list'),
                array('name' => '投诉管理', 'model'=>'store_complain', 'fun'=>'list'),
                array('name' => '退款记录', 'model'=>'store_refund', 'fun'=>'index'),
                array('name' => '退货记录', 'model'=>'store_return', 'fun'=>'index'),
            )),
            'statistics' => array('name' => '统计结算', 'child' => array(
                array('name' => '商城概况', 'model'=>'statistics_general', 'fun'=>'general'),
                array('name' => '商品分析', 'model'=>'statistics_goods', 'fun'=>'goodslist'),
                array('name' => '运营报告', 'model'=>'statistics_sale', 'fun'=>'sale'),
//                 array('name' => '行业分析', 'model'=>'statistics_industry', 'fun'=>'hot'),
//                 array('name' => '流量统计', 'model'=>'statistics_flow', 'fun'=>'storeflow'),
//                 array('name' => '商品结算', 'model'=>'store_bill', 'fun'=>'index'),
//                 array('name' => '虚拟结算', 'model'=>'store_vr_bill', 'fun'=>'index'),
            )),
//             'message' => array('name' => '客服消息', 'child' => array(
//                 array('name' => '客服设置', 'model'=>'store_callcenter', 'fun'=>'index'),
//                 array('name' => '系统消息', 'model'=>'store_msg', 'fun'=>'index'),
//                 array('name' => '聊天记录查询', 'model'=>'store_im', 'fun'=>'index'),
//             )),
//             'account' => array('name' => '账号', 'child' => array(
//                 array('name' => '账号列表', 'model'=>'store_account', 'fun'=>'account_list'),
//                 array('name' => '账号组', 'model'=>'store_account_group', 'fun'=>'group_list'),
//                 array('name' => '账号日志', 'model'=>'seller_log', 'fun'=>'log_list'),
//                 array('name' => '店铺消费', 'model'=>'store_cost', 'fun'=>'cost_list'),
//                 array('name' => '门店账号', 'model'=>'store_chain', 'fun'=>'index'),
//             ))
			//临时注释
			/*'webchat' => array('name' => '微信', 'child' => array(
                array('name' => '微信接口管理', 'model'=>'seller_wechat', 'fun'=>'index'),
                array('name' => '关注自动回复', 'model'=>'seller_wechat_follow', 'fun'=>'follow_index'),
                array('name' => '关键词自动回复', 'model'=>'seller_wechat_keyword', 'fun'=>'keyword_index'),
                array('name' => '消息自动回复', 'model'=>'seller_wechat_message', 'fun'=>'message_index'),
                array('name' => '自定义菜单', 'model'=>'seller_wechat_menu', 'fun'=>'index'),
            ))*/
        );
        return $menu_list;
    }
    ...
    private function _getSellerFunctionList($menu_list) {
        ...
        foreach ($menu_list as $key => $menu_value) {
            foreach ($menu_value['child'] as $submenu_value) {
                $format_menu[$submenu_value['model']] = array(
                    'model' => $key,
                    'model_name' => $menu_value['name'],
                    'name' => $submenu_value['name'],
                    'act' => $submenu_value['model'],
                    'op' => $submenu_value['fun'],
                );
            }
        }
        return $format_menu;
...
class BasePointShopControl extends Control {
    ...
    public function __construct(){
        Language::read('common,home_layout,home_pointprod,member_layout');
        //输出头部的公用信息
        $this->showLayout();
        //输出会员信息
        $this->member_info = $this->getMemberAndGradeInfo(true);
