    ...
    public function step2Op() {
        ...
        if (chksubmit()){
            ...
            if(!$_SESSION['member_id']){
                showDialog('未登录','','error');
                //exit(json_encode(array('state'=>false,'msg'=>'未登录')));
            }

            if (!checkSeccode($_POST['nchash'],$_POST['captcha'])){
                setNcCookie('seccode'.$_POST['nchash'],'',-3600);
                showDialog('Verification code error','reload','error');
                //showDialog('Verification code error','','error');
            }
            //print_R($_POST);die;
            ...
            $condition['jiameng_time'] = array('egt',$time24);
            $count = $model_store_joinin->getMessageCount($condition);
//var_dump($count);die;
            if($count>=5){
                showDialog('You have submitted messages too many times today! Do not duplicate submissions','','error');
            ...
            $param['company_address'] = $_POST['address'];
            ...
            // 检查一下参数是否正确
            $this->step2_save_valid2($newParam);

            $param['leave_message'] = serialize($newParam);
            //$param['member_id'] = $this->member_info['member_id'];
            //$model_store_joinin->save($param);
            $result = $model_store_joinin->save($param);
            ...
            //}
            showDialog('Message successfully','index.php?model=contact&fun='.$_POST['type'],'succ');
        }
    ...
    private function step2_save_valid($param) {
        ...
        $obj_validate->validateparam = array(
            ...
            array("input"=>$param['contacts_name'], "require"=>"true","message"=>"Contact name cannot be empty"),
            array("input"=>$param['contacts_phone'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"Contact calls cannot be empty"),
            array("input"=>$param['contacts_email'], "require"=>"true","validator"=>"email","message"=>"E-mail cannot be empty"),
            array("input"=>$param['jiameng_message'],"validator"=>"Length","min"=>"1","max"=>"500","message"=>"Messages should not be empty and must be less than 500 words"),
        );
    ...
    private function step2_save_valid2($param) {
        $obj_validate = new Validate();
        //print_R($param);die;
        $obj_validate->validateparam = array(
            // array("input"=>$param['company_name'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"Shop name不能为空且必须小于50个字"),
            ...
            array("input"=>$param['last_name'], "require"=>"true","message"=>"Last name cannot be empty"),
            array("input"=>$param['city'], "require"=>"true","message"=>"City cannot be empty"),
            array("input"=>$param['code'], "require"=>"true","message"=>"ZIP Code or Postal Code cannot be empty"),
            array("input"=>$param['province'], "require"=>"true","message"=>"State or Province cannot be empty"),
            array("input"=>$param['info1'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"Contact calls cannot be empty"),
        );
        $error = $obj_validate->validate();
        if ($error != ''){
            // showMessage($error);
    ...
    private function step4_save_valid($param) {
        $obj_validate = new Validate();
        $obj_validate->validateparam = array(
            array("input"=>$param['store_name'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"Shop name不能为空且必须小于50个字"),
