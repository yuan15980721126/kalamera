...
class store_goods_addControl extends BaseSellerControl {
    ...
    private function checkStore(){
        ...
        if ($goodsLimit > 0) {
            ...
            if ($goods_num >= $goodsLimit) {
                showMessage(L('store_goods_index_goods_limit') . $goodsLimit . L('store_goods_index_goods_limit1'), 'index.php?model=store_goods_online&fun=goods_list', 'html', 'error');
            }
        }
    }
    ...
    public function add_step_twoOp() {
        ...
        if (!$_SESSION['seller_gc_limits']&& $_SESSION['seller_group_id']) {
            ...
            if (!$rs) {
                showMessage('Your group does not have the authority to operate on items under this category', '', 'html', 'error');///您所在的组无权操作该分类下的商品
            }
        }
    ...
    public function file_uploadOp() {
        $logic_goods = Logic('goods');
        $result =  $logic_goods->uploadGoodsFile(
            $_POST['name'],
            $_SESSION['store_id']
        );
