    ...
    public function fcode_quota_addOp() {
        if (chksubmit()) {
            ...
            if ($quantity <= 0 || $quantity > 12) {
                showDialog('Parameter error，购买失败。', urlShop('store_promotion_fcode', 'fcode_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_fcode = Model('p_fcode');
    ...
    public function choosed_goodsOp() {
        ...
        if (chksubmit()) {
            ...
            if ($rs) {
                ...
                showDialog('Operation is successful', '', 'succ', 'CUR_DIALOG.close();choose_goods('.json_encode($goods_info).')');
            } else {
