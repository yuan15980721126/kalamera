...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function goods_listOp() {
        ...
        $goods_list = $model_goods->getGoodsCommonOnlineList($where,'','','sort asc');
        // echo "<pre>";
        // print_r($goods_list);
    ...
    public function edit_goodsOp() {
        ...
        if (!$_SESSION['seller_gc_limits'] && $_SESSION['seller_group_id']) {
            ...
            if (!in_array($goodscommon_info['gc_id'],array_keys($gc_list))) {
                showMessage('Your group does not have the authority to operate on items under this category','', 'html', 'error');
            }
        }
        ...
        /*
        // 文件下载
        $model_ingredient = Model('goods_ingredient');
        $ingredient_array = $model_ingredient->getIngredList(array('goods_commonid' => $common_id, 'store_id' => $goodscommon_info['store_id']));
        if (empty($ingredient_array)) {
            $ingredient_array[] = array('goods_ingredient_id' => '0');
        }
        */
    ...
    public function edit_imageOp() {
        ...
        $file_array = unserialize($common_list['goods_files']);
        
        Tpl::output('files', $file_array);

        $repair_array = unserialize($common_list['goods_repair']);

        Tpl::output('repair', $repair_array);
    ...
    public function edit_save_imageOp() {
        if (chksubmit()) {
            ...
            $rs2 = Logic('goods')->editSaveFile($_POST['files'], $common_id, $_SESSION['store_id'], $_SESSION['seller_id'], $_SESSION['seller_name']);
            if (!$rs2['state']) {
                showDialog(L('nc_common_save_fail'), urlShop('store_goods_online', 'index'));
            }

            $rs3 = Logic('goods')->editSaveRepair($_POST['repair'], $common_id, $_SESSION['store_id'], $_SESSION['seller_id'], $_SESSION['seller_name']);
            if (!$rs3['state']) {
                showDialog(L('nc_common_save_fail'), urlShop('store_goods_online', 'index'));
            }
    ...
    public function goods_unshowOp() {
        ...
        if ($result['state']) {
            showDialog(L('store_goods_index_goods_unshow_success'), getReferer() ? getReferer() : 'index.php?model=store_goods_online&fun=goods_list', 'succ', '', 2);
        } else {
        ...
        switch ($menu_type) {
            ...
            case 'edit_detail':
                if ($allow_promotion['lock'] === false) {
                    ...
                    //    array('menu_key' => 'edit_file',  'menu_name' => '编辑文件', 'menu_url' => urlShop('store_goods_online', 'edit_file', array('commonid' => $_GET['commonid'], 'ref_url' => ($_GET['ref_url'] ? $_GET['ref_url'] : getReferer())))),
                    );
                }
