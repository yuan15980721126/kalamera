...
class store_contractControl extends BaseSellerControl {
    ...
    public function __construct() {
        ...
        if (C('contract_allow') != 1){
            showDialog('需开启“消费者保障服务”功能','index.php?model=seller_center&fun=index','error');
        }
    ...
    public function indexOp(){
        ...
        if (!$item_list) {
            showDialog('平台尚未开启任何消费者保障服务','index.php?model=seller_center&fun=index','error');
        }
        //查询店铺保障项目开启情况
    ...
    private function applyadd_platformstore($item_info, $c_info){
        ...
        try {
            ...
            showDialog('加入成功','index.php?model=store_contract&fun=index','succ');
        }catch (Exception $e){
    ...
    private function applyadd_joininstore($item_info, $c_info){
        ...
        try {
            ...
            $model_contract->commit();
            showDialog('加入申请成功，请耐心等待审核结果','index.php?model=store_contract&fun=index','succ');
        }catch (Exception $e){
            $model_contract->rollback();
            showDialog($e->getMessage(), '', 'error');
        }
    ...
    public function applypayOp(){
        ...
        if(chksubmit()){
            try {
                ...
                if (!$result) {
                    throw new Exception('支付保证金The operation failure');
                }
                $update_arr = array();
                ...
                $model_contract->commit();
                showDialog('支付保证金Operation is successful，请耐心等待审核结果','index.php?model=store_contract&fun=index','succ');
            }catch (Exception $e){
                $model_contract->rollback();
                showDialog($e->getMessage(), '', 'error');
            }
    ...
    private function applyquit_platformstore($item_info, $c_info){
        ...
        try {
            ...
            $model_contract->commit();
            QueueClient::push('updateStoreGoodsContract', array('store_id'=>$_SESSION['store_id'],'item_id'=>$item_info['cti_id']));
            showDialog('退出成功','index.php?model=store_contract&fun=index','succ');
        }catch (Exception $e){
            $model_contract->rollback();
            showDialog($e->getMessage(), '', 'error');
        }
    ...
    private function applyquit_joininstore($item_info, $c_info){
        ...
        try {
            ...
            $model_contract->commit();
            showDialog('退出申请提交成功，请耐心等待审核结果','index.php?model=store_contract&fun=index','succ');
        }catch (Exception $e){
            $model_contract->rollback();
            showDialog($e->getMessage(), '', 'error');
        }
   ...
   /**
    ...
    private function profile_menu($menu_type='',$menu_key='') {
        ...
        switch ($menu_type) {
            case 'log':
                $menu_array = array(
                    1=>array('menu_key'=>'index', 'menu_name'=>'消费者保障服务', 'menu_url'=>'index.php?model=store_contract&fun=index'),
