...
class store_promotion_bookControl extends BaseSellerControl {
    ...
    public function book_quota_addOp() {
        if (chksubmit()) {
            ...
            if ($quantity <= 0 || $quantity > 12) {
                showDialog('Parameter error，购买失败。', urlShop('store_promotion_book', 'book_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_book = Model('p_book');
    ...
    public function choosed_goodsOp() {
        ...
        if ($gid <= 0) {
            if ($_GET['inajax']) {
                showDialog('Parameter error', '', 'succ', 'CUR_DIALOG.close();');
            }
            Tpl::output('error', 'Parameter error');
        }
        ...
        if (empty($goods_info) || $goods_info['store_id'] != $_SESSION['store_id'] || !$model_goods->checkIsGeneral($goods_info)) {
            if ($_GET['inajax']) {
                showDialog('Parameter error，或该商品已经添加过活动', '', 'succ', 'CUR_DIALOG.close();');
            }
            Tpl::output('error', 'Parameter error，或该商品已经添加过活动');
        }
        if (chksubmit()) {
            $update = array();
            if ($_POST['type'] == 'book') {
            ...
            if ($rs) {
                ...
                showDialog('Operation is successful', '', 'succ', 'CUR_DIALOG.close();choose_goods('.json_encode($goods_info).', "'. $_POST['type'] .'")');
            } else {
                showDialog('The operation failure', '', 'succ', 'CUR_DIALOG.close();');
            }
        }                
    ...
    public function del_choosed_goodsOp() {
        $gid = intval($_GET['gid']);
        if ($gid <= 0) {
            $data = array('result' => 'false', 'msg' => 'Parameter error');
            $this->_echoJson($data);
        }
        ...
        if ($result) {
            ...
            $data = array('result' => 'false', 'msg' => 'Delete failed');
        }
        $this->_echoJson($data);
    }
