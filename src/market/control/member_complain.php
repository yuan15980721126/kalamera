...
class member_complainControl extends BaseMemberControl{
    ...
    public function complain_saveOp() {
        ...
        if($if_complain < 1) {
            showDialog(Language::get('para_error'),'index.php?model=member_order&fun=index','error');
        }
        //检查是不是正在进行投诉
        ...
        if ($state) {
            showDialog(Language::get('complain_submit_success'),'index.php?model=member_complain&fun=index','succ');
        } else {
