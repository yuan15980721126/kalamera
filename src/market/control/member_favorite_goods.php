...
class member_favorite_goodsControl extends BaseMemberControl{
    ...
    public function favoritegoodsOp(){
        ...
        if(!empty($favorites_info)){
            echo json_encode(array('done'=>false,'msg'=>'You have collected the goods.'));
            die;
        }
        //判断商品是否为当前会员所有
    ...
    public function fglistOp(){
        ...
        if (trim($_GET['goods_name']) != '' ){
            $name = trim($_GET['goods_name']);
            $condition['goods_name'] = array('like','%'.$name.'%');
        }
        $favorites_list = $favorites_model->getGoodsFavoritesList($condition, '*', 12);
        ...
        $cart_commend_list = $model_goods->getGoodsCommendList(1, 4);
        Tpl::output('cart_commend',$cart_commend_list);

        Tpl::output('show_page', $favorites_model->showpage());
        $nav_link = array(
            array(
                'title'=>'My Account',
        ...
        if (!empty($fav_arr) && is_array($fav_arr)){
            ...
            if (!empty($favorites_list) && is_array($favorites_list)){
                ...
                if (!empty($fav_arr) && $result){
                    ...
                    showDialog(Language::get('favorite_del_success'),'index.php?model=member_favorite_goods&fun=fglist&show='.$_GET['show'],'succ');
                }
            }else {
    ...
    private function profile_menu($menu_type,$menu_key='') {
        $menu_array = array(
            array('menu_key'=>'fav_goods','menu_name'=>'Favorites', 'menu_url'=>'index.php?model=member_favorite_goods&fun=fglist'),
            // array('menu_key'=>'fav_goodsstore','menu_name'=>'同店商品', 'menu_url'=>'index.php?model=member_favorite_goods&fun=fglist&show=store'),
        );
