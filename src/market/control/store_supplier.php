...
class store_supplierControl extends BaseSellerControl {
    ...
    public function sup_saveOp(){
        ...
        if ($result){
            showDialog('Save successfully','reload','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
        }else {
    ...
    public function sup_delOp() {
        ...
        if ($sup_id > 0){
            ...
            showDialog(Language::get('nc_common_del_succ'),'index.php?model=store_supplier&fun=sup_list','succ');
        }else {
    ...
    private function profile_menu($menu_type, $menu_key = '', $array = array()) {
        ...
        switch ($menu_type) {
        	case 'sup_list':
        	    $menu_array = array(
        	    array('menu_key'=>'sup_list', 'menu_name'=>'供货商', 'menu_url'=>'index.php?model=store_supplier&fun=sup_list')
        	    );
        	    break;
        }
        if(!empty($array)) {
