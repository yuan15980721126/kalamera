...
class store_joininControl extends BaseHomeControl {
            ...
            switch (intval($joinin_detail['joinin_state'])) {
                ...
                case STORE_JOIN_STATE_VERIFY_FAIL:
                    if(!in_array($_GET['fun'], array('step1', 'step2', 'step3', 'step4','ajax_upload_image'))) {
                        $this->show_join_message('审核失败:'.$joinin_detail['joinin_message'], SHOP_SITE_URL.DS.'index.php?model=store_joinin&fun=step1');
                    }
                    break;
                case STORE_JOIN_STATE_PAY_FAIL:
                    if(!in_array($_GET['fun'], array('pay', 'pay_save'))) {
                        $this->show_join_message('付款审核失败:'.$joinin_detail['joinin_message'], SHOP_SITE_URL.DS.'index.php?model=store_joinin&fun=pay');
                    }
                    break;
    ...
    public function pay_saveOp() {
        ...
        if(empty($param['paying_money_certificate'])) {
            showMessage('Please upload the payment voucher','','','error'); //请上传付款凭证
        }
