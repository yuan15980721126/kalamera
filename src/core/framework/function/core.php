...
function requestNotFound($act = null, $op = null) {
    showMessage('Sorry! Page not found.', SHOP_SITE_URL, 'exception', 'error', 1, 3000);
    exit;
}
...
function getCMSArticleUrl($article_id) {
    if(URL_MODEL) {
        ...
        return CMS_SITE_URL.DS.'index.php?model=article&fun=article_detail&article_id='.$article_id;
    }
}
...
function getCMSPictureUrl($picture_id) {
    if(URL_MODEL) {
        ...
        return CMS_SITE_URL.DS.'index.php?model=picture&fun=picture_detail&picture_id='.$picture_id;
    }
}
...
function getCMSSpecialUrl($special_id) {
    return CMS_SITE_URL.DS.'index.php?model=special&fun=special_detail&special_id='.$special_id;
}
...
function getShopSpecialUrl($special_id) {
    return SHOP_SITE_URL.DS.'index.php?model=special&fun=special_detail&special_id='.$special_id;
}
 ...
 *
 * url('search,'index','array('cate_id'=>2)); 实际指向 index.php?model=search&fun=index&cate_id=2
 * 传递数组参数时，若act（或op）值为index,则可以省略
 * 上面示例等同于
 * url('search','',array('model'=>'search','cate_id'=>2));
 *
 ...
 * @param  $ap_id 广告位ID
 */
function loadadvlist($ap_id = null){
    if (!is_numeric($ap_id)) return false;
    if (!function_exists('advshow')) import('function.adv');
    return advlist($ap_id,$type);
}
