...
class evaluate_goodsModel extends Model {
    ...
    public function getEvaluateGoodsInfoByGoodsID($goods_id) {
        ...
        if(empty($info)) {
            ...
            if(intval($info['all']) > 0) {
                ...
                $info['good_star'] = $info['good'] / $info['all'] * 5;
                $info['star_average'] = ($star1 + $star2 * 2 + $star3 * 3 + $star4 * 4 + $star5 * 5) / $info['all'];
            } else {
     ...
     * @return int
     */
    public function getEvaluateGoodsCount($condition) {
     ...
     * @param array $condition
     */
    public function getEvaluateInfo($condition = array()) {
    ...
    public function addEvaluateGoodsArray($param, $goodsid_array) {
        ...
        return $result;
          ...
          *新增插件评价添加插件 所需上传通道
            **/
    public function addEvaluateGoods($insert,$goodsid='') {
        $result = $this->insert($insert);
        if ($result && !empty($goodsid)) {
            dcache($goodsid, 'goods_evaluation');
        }
        return $result;
    }
    /**
