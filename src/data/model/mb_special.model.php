...
class mb_specialModel extends Model{
    ...
    private function _initMbSpecialItemGoodsData($item_data, $item_type) {
        ...
        if(!empty($item_data['item'])) {
            ...
            //print_R($goods_list);
            //print_R($item_data['item']);
            //整理商品数据
            $new_goods_list = array();
            foreach ($item_data['item'] as $value) {
                $goods_info = $model_goods->getGoodsInfoAndPromotionById($value);
                //print_R($goods_info);

                if(!empty($goods_list[$value])) {
                    $goods_list[$value]['promotion_type'] = '';
                    if(!empty($goods_info['xianshi_info'])){
                        $goods_list[$value]['promotion_type'] = 'xianshi';
                        $goods_list[$value]['goods_promotion_price'] = $goods_info['xianshi_info']['xianshi_price'];
                    }
                    if(!empty($goods_info['groupbuy_info'])){
                        $goods_list[$value]['promotion_type'] = 'groupbuy';
                        $goods_list[$value]['goods_promotion_price'] = $goods_info['groupbuy_info']['groupbuy_price'];
                    }
