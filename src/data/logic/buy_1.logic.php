...
class buy_1Logic {
    ...
    public function parseZhekou(& $goods_list, $store_order_discount = array()) {
        ...
        if (array_key_exists('store_id', $goods_list)) {
            if (array_key_exists($goods_list['store_id'], $store_order_discount)) {
                //直接购买（单个商品）
                if (!$goods_list['is_book'] && !$goods_list['groupbuy_info'] && !$goods_list['xianshi_info'] && !isset($goods_list['jjgRank'])) {
                    $goods_list['user_discount']  = $orderdiscount;
                    $goods_list['user_discountdesc']  = $orderdiscount/10;
            ...
            foreach ($goods_list as $k => $goods_info) {
                if (array_key_exists($goods_info['store_id'], $store_order_discount)) {
                    ...
                    if(($goods_info['groupbuy_info'] == '' && $goods_info['xianshi_info'] == '') && !isset($goods_info['jjgRank'])){
                        ...
                        $goods_list[$k]['user_discount']  = $orderdiscount;
                        $goods_list[$k]['user_discountdesc']  = $orderdiscount/10;
    ...
    public function getReciverAddr($address_info = array()) {
        if (intval($address_info['dlyp_id'])) {
            ...
            $reciver_info['state'] = $address_info['state'];
            $reciver_info['zipcode'] = $address_info['zipcode'];
            $reciver_info['city'] = $address_info['city'];
            $reciver_info['dlyp'] = 1;
            ...
            $reciver_info['address'] = $address_info['address'].' '.str_replace(array("\r","\n"),',',$address_info['area_info']); //美国地址由小到大 by siuloong 20190506
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        $goods_common =$model_goods->getGoodsCommonInfoByID($goods_info['goods_commonid']);
//    echo '<pre>';
//    print_r($goods_common);die;
        $new_array = array();
        ...
        $new_array['goods_repair'] = $goods_common['goods_repair'];
