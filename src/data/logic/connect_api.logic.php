...
class connect_apiLogic {
    ...
    public function getWxOAuth2Url(){
        ...
        $api_url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$weixin_appid.
            '&redirect_uri='.urlencode(MOBILE_SITE_URL.'/index.php?model=connect&fun=index').
     ...
     * @author siuloong
     */
    public function getFacebookUserInfo($fb_userid)
    {
     ...
     * @author siuloong
     */
    public function facebookRegister($user_info, $client = 'www')
    {
        $state_data = array();
        $state_data['token'] = 0;
        $fb_userid = $user_info['user_id'];
        $info_str = serialize($user_info);
        $rand = rand(100, 899);
        $member_name = $this->getMemberName($fb_userid, $rand);
        $password = md5($member_name . rand(100, 999));

        $member = array();
        $member['member_name'] = $member_name;
        $member['member_passwd'] = $password;
        $member['member_email'] = $member_name . '@example.com';
        $member['facebook_userid'] = $fb_userid;
        $member['facebook_userinfo'] = $info_str;
        $model_member = Model('member');
        $result = $model_member->addMember($member);
