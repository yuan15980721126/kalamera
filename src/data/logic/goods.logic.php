...
class goodsLogic {
    ...
    private function _initCommonGoodsByParam($param, $store_id, $store_name, $store_state) {
        ...
        $common_array['sort']               = $param['g_sort'];
        ...
        $common_array['goods_quality']      = $param['goods_quality'];//主要特点
        $common_array['goods_guarantee']    = $param['goods_guarantee'];//满意保证
        $common_array['goods_spec_desc']    = $param['goods_spec_desc'];//规格描述
        $common_array['goods_tixing'] =     $param['goods_tixing'];//提醒
    ...
    private function _addGoods($param, $common_id, $common_array) {
        ...
        if (is_array($param['spec'])) {
            foreach ($param['spec'] as $value) {
                ...
                $goods['sort']              = intval($param['g_sort']);
    ...
    public function uploadGoodsImage($image_name, $store_id, $album_limit)
    {
        ...
        $result = $upload->upfile($image_name,true, true);
        if (!$result) {
    ...
    /**
     * 上传文件
     * @author siuloong 20190430
     */
    public function uploadGoodsFile($file_name, $store_id)
    {
        // 上传图片
        $upload = new UploadFile();
        $upload->set('default_dir', ATTACH_STORE . DS . 'goods_files' . DS . $store_id . DS . $upload->getSysSetPath());
        $upload->set('max_size', 20 * 1024); //C('file_max_filesize')

        $upload->set('fprefix', $store_id);
        $upload->set('allow_type', array('doc', 'docx', 'pdf', 'txt'));
        $result = $upload->upfile($file_name, true);
        if (!$result) {
            return callback(false, $upload->error);
        }

        $file_path = $upload->getSysSetPath() . $upload->file_name;

        //$full_url = '/data/upload/' . $upload->get('default_dir') . $file_path;

        $data = array ();
        $data['thumb_name'] = '/data/upload/shop/common/file.png';
        $data['name']      = $file_path;
     ...
     * @author siuloong 20190430
     */
    public function editSaveFile($files, $common_id, $store_id, $seller_id, $seller_name) {

        if ($common_id <= 0 || empty($_POST['files'])) {
            return callback(false, '参数错误');
        }
        ...
        $newFunc = create_function('$n', 'return $n["sort"];');
        $arr1 = array_map($newFunc, $file_array);
        array_multisort($arr1, $file_array );//多维数组的排序


        $update_array = array('goods_files' => serialize($file_array));
        $rs = $model_goods->editGoodsCommonById($update_array, array($common_id));
        if ($rs) {
            $this->_recordLog('商品文件编辑，SPU:'.$common_id, $seller_id, $seller_name, $store_id);
            return callback(true);
        } else {
            return callback(false, '商品文件编辑失败');
        }
    }
     ...
     * @author siuloong 20190430
     */
    public function editSaveRepair($repair, $common_id, $store_id, $seller_id, $seller_name) {

        if ($common_id <= 0 || empty($repair)) {
            return callback(false, '参数错误');
        }
        ...
        foreach ($repair as $key => $value) {
            if (!$value['description'] &&  !$value['percent']) {
                continue;
            }
            $repair_array[] = array(
                'sort' => $value['sort'],
        ...
        $newFunc = create_function('$s', 'return $s["sort"];');
        $arr2 = array_map($newFunc, $repair_array);
        array_multisort($arr2, $repair_array );//多维数组的排序

        $update_array = array('goods_repair' => serialize($repair_array));
        $rs = $model_goods->editGoodsCommonById($update_array, array($common_id));
        if ($rs) {
            $this->_recordLog('商品保修政策编辑，SPU:'.$common_id, $seller_id, $seller_name, $store_id);
            return callback(true);
        } else {
            return callback(false, '商品保修政策编辑失败');
        }
    }
