...
//         print_R($store_cart_list);die;
    ...
    public function getGoodsList($cart_id, $member_id, $store_id,$orderdiscounts = array()) {
        ...
        //Cant buy goods from your shop
        if ($goods_info['store_id'] == $store_id) {
            return callback(false, 'Cant buy goods from your shop');
        }
...
//             $order_platform_store_ids[$k] = null;
        ...
        $data['tax'] = $this->get_tax($city_id,$area_id);
    ...
    /**
     * 选择不同地区时，异步处理并返回每个城市的税金
     */
    public function get_tax($city_id,$area_id ) {
        $model_area = Model('area');

        $area_city = $model_area->getAreaInfo(array('area_id' =>$city_id));
        $tax = 0;
        if($area_city['tax'] > 0){
            $tax = $area_city['tax'];
        }
...
//             print_R($input_buy_items);die;
        ...
        if (!$post['chain']['id']) {
            ...
            if ($chain_info) {
                ...
                //收获州编号 by siuloong 20190506
                $input_area_id = intval($input_address_info['area_id_1']);     
...
//        die;
        ...
        $this->_order_data['input_tax'] = ncPriceFormat($post['offpay_tax']);
        $this->_order_data['input_tax_payment'] = ncPriceFormat($post['offpay_tax_payment']);
...
//            Log::record('重新计算店铺扣除优惠券送商品实际支付金额'.json_encode($store_final_goods_total),Log::ERR);
    ...
    private function _createOrderStep4() {
        ...
        if (!$order_pay_id) {
            throw new Exception('订单Save failed[未生成支付单]');
        }
...
//            Log::record('税额'.json_encode($input_tax),Log::ERR);
            ...
            if (!$order_id) {
                throw new Exception('订单Save failed[未生成订单数据]');
            }
            ...
            $order_common['reciver_province_id'] = $input_area_id; // 收货州 by siuloong 20190506
            ...
            //保存订单州份和代号信息
            if($input_address_info['city_id']){
                $model_area = Model('area');
                $area_state = $model_area->getAreaInfo(array('area_id' =>$input_address_info['city_id']));
                $area_city = $model_area->getAreaInfo(array('area_id' =>$input_address_info['area_id']));
                $order_common['area_name'] = $area_city['area_name'];
                if($area_state['area_parent_id']){
                    $area_parent = $model_area->getAreaInfo(array('area_id' =>$area_state['area_parent_id'],'area_parent_id'=>0));
                    $order_common['area_code'] = $area_parent['area_code'];
                }
            }
            //保存促销信息
...
//                Log::record('优惠券备注信息'.json_encode($voucher_arr),Log::ERR);
            ...
            if (!$insert) {
                throw new Exception('订单Save failed[未生成订单扩展数据]');
            }
            ...
            foreach ($goods_list as $goods_info) {
                ...
                if (!intval($goods_info['bl_id'])) {
                    ...
                    $order_goods[$i]['goods_repair'] = serialize($goods_info['goods_repair']);
                    ...
                    foreach ($goods_info['bl_goods_list'] as $bl_goods_info) {
                        ...
                        $order_goods[$i]['goods_repair'] = serialize($bl_goods_info['goods_repair']);
            ...
            if (!$insert) {
                throw new Exception('订单Save failed[未生成商品数据]');
            }
    ...
    private function _createOrderStep6() {
        ...
        if (!$result['state']) {
            throw new Exception('订单Save failed[变更库存销量失败]');
        }
        ...
        if ($input_chain_id) {
            ...
            if (!$result['state']) {
                throw new Exception('订单Save failed[变更自提门店库存销量失败]');
            }
        }
        ...
        if (!empty($input_voucher_list) && is_array($input_voucher_list)) {
            ...
            if (!$result['state']) {
                throw new Exception('订单Save failed[代金券处理失败]');
            }
        }
        ...
        if (!empty($input_rpt_info) && is_array($input_rpt_info)) {
            ...
            if (!$result['state']) {
                throw new Exception('订单Save failed[平台红包处理失败]');
            }
        }
        ...
        if ($fc_id) {
            ...
            if (!$result['state']) {
                throw new Exception('订单Save failed[F码处理失败]');
            }
        }
...
//        print_r($cart_list);die;
        foreach ($cart_list as $key => $cart) {
            ...
            if (!intval($cart['bl_id'])) {
                ...
                $goods_list[$i]['goods_repair'] = $cart['goods_repair'];
                $i++;
            } else {
                //如果是优惠套装商品
