...
$(function (){
  ...
  function get_detail(goods_id) {
      //渲染页面
      $.ajax({
         url:ApiUrl+"/index.php?model=goods&fun=goods_detail",
         type:"get",
            ...
            if(!data.error){
              ...
              $(".minus").click(function (){
                 ...
                 if(buynum >1){
                    ...
                    $(".buy-num").text(parseInt(buynum-1));
                 }
              });
              //购买数量加
              $(".add").click(function (){
                 var buynum = parseInt($(".buy-num").val());
                 var buynum = parseInt($(".buy-num").text());
              ...
              $("#add-cart").click(function (){
                var key = getCookie('key');//登录标记
                var quantity = parseInt($("#De_html").text());
                if(!key){
                     var goods_info = decodeURIComponent(getCookie('goods_cart'));
                     if (goods_info == null) {
                         goods_info = '';
              ...
              if (data.goods_info.is_virtual == '1') {
                  $("#buy-now").click(function() {
                      ...
                      var buynum = parseInt($('#De_html').text()) || 0;
                      ...
                      $.ajax({
                          type:'post',
                          url:ApiUrl+'/index.php?model=member_vr_buy&fun=buy_step1',
                          data:json,
                          dataType:'json',
                          success:function(result){
...
//$.scrollTransparent();
  $('#product_detail_html').on('click', '#get_area_selected', function(){
      $.areaSelected({
          success : function(data){
              ...
              $.getJSON(ApiUrl + '/index.php?model=goods&fun=calc', {goods_id:goods_id,area_id:area_id},function(result){
  ...
  $('body').on('click', '#consultadd', function(){
      window.location.href = WapSiteUrl+'/tmpl/member/BuyQu.html?goods_id=' + goods_id;
  });
});
...
function virtual() {
    ...
    $.getJSON(ApiUrl + '/index.php?model=goods&fun=store_o2o_addr', {store_id:store_id},function(result){
    	if (!result.datas.error) {
...
$("#product_detail_html").on("click",".ran_btn",function(){
		$("#product_detail_html .ran_btn").removeClass("currange");
		$(this).addClass("currange");
	});
	
	var totalnub=1;
	$("#product_detail_html").on("click","#De_add",function(){
	totalnub++
		$("#De_html").html(totalnub)
	
	});
$("#product_detail_html").on("click","#De_down",function(){
	if(totalnub==1){
		return false;
	}
	totalnub--;
