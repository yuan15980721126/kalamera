...
class SystemControl{
    ...
    protected function __construct() {
        ...
        if (($_GET['branch']!='' || $_GET['fun']=='ajax') && strtoupper(CHARSET) == 'GBK'){
    ...
    protected final function systemLogin() {
        ...
        if (!key_exists('gid',(array)$user) || !isset($user['sp']) || (empty($user['name']) || empty($user['id']))){
            @header('Location: '. ADMIN_SITE_URL .'/index.php?model=login&fun=login');exit;
        }else {
    ...
    protected final function checkPermission($link_nav = null) {
        ...
        $act = $_GET['model'] ? $_GET['model'] : $_POST['model'];
        $act = $act == '' ? 'index' : $act;
    ...
    protected final function log($lang = '', $state = 1, $admin_name = '', $admin_id = 0) {
        ...
        $data['ip']         = getIp();
        $data['url']        = $_REQUEST['model'].'&'.$_REQUEST['fun'];
