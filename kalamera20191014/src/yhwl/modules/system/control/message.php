...
class messageControl extends SystemControl{
    private $links = array(
        array('url'=>'model=message&fun=email','lang'=>'email_set'),
        array('url'=>'model=message&fun=email_tpl','lang'=>'email_tpl')
    );
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result === true){
                    ...
                    showMessage(L('mailtemplates_edit_succ'),'index.php?model=message&fun=email_tpl');
                }else {
