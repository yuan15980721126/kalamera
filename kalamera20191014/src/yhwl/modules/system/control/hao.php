...
class haoControl extends SystemControl{
	 private $links = array(
	    array('url'=>'model=hao&fun=base','lang'=>'hao_set'),
        array('url'=>'model=hao&fun=banner','lang'=>'top_set'),
        array('url'=>'model=hao&fun=lc','lang'=>'lc_set'),
		array('url'=>'model=hao&fun=sms','lang'=>'sms_set'),
		//临时注释
		//array('url'=>'model=hao&fun=rc','lang'=>'rc_set'),
		array('url'=>'model=hao&fun=webchat','lang'=>'webchat_set'),
    ...
    public function lc_addOp() {
        ...
        if (chksubmit()) {
            if (count($lc_list) >= 8) {
                showMessage('最多可设置8个楼层','index.php?model=hao&fun=lc');
            }
            ...
            if ($result){
                showMessage('Save successfully','index.php?model=hao&fun=lc');
            }else {
    ...
    public function lc_editOp() {
        ...
        if (!chksubmit()) {
            ...
            $result = $model_setting->updateSetting(array('hao_lc'=>serialize($lc_list)));
            if ($result){
                showMessage('编辑成功','index.php?model=hao&fun=lc');
            }
            showMessage('编辑失败');
        }
    ...
    public function rc_addOp() {
        ...
        if (chksubmit()) {
            if (count($rc_list) >= 8) {
                showMessage('最多可设置8个楼层','index.php?model=hao&fun=rc');
            }
            ...
            if ($result){
                showMessage('Save successfully','index.php?model=hao&fun=rc');
            }else {
                showMessage('Save failed');
            }
        }
    ...
    public function rc_editOp() {
        ...
        if (!chksubmit()) {
            ...
            $result = $model_setting->updateSetting(array('hao_rc'=>serialize($rc_list)));
            if ($result){
                showMessage('编辑成功','index.php?model=hao&fun=rc');
            }
            showMessage('编辑失败');
        }
