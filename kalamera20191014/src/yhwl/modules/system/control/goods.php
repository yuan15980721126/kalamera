...
class goodsControl extends SystemControl{
    private $links = array(
        array('url'=>'model=goods&fun=goods','text'=>'所有商品'),
    );
    ...
    public function addOp(){
        ...
        if (chksubmit()){
            ...
            $order['buyer_email'] = 0;
            $order['buyer_phone'] = 0;
            $order['add_time'] = empty($_POST['add_time']) ? TIMESTAMP- (mt_rand(3,10) * 24 * 3600) : strtotime($_POST['add_time']) - (mt_rand(3,10) * 24 * 3600);
            ...
            $model_order->addOrderGood($order_goods);
				...
				//添加商品评分 
                 ...
                 $insert_array['geval_addtime'] = empty($_POST['add_time']) ? TIMESTAMP : strtotime($_POST['add_time']);
                ...
                $evaluate_store_info['seval_addtime'] = empty($_POST['add_time']) ? TIMESTAMP : strtotime($_POST['add_time']);
            ...
            $result = Model('goods')->editGoodsById($data, $_POST['goodsid']);
                 ...
                 if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?model=goods&fun=add&gid='.$_POST['goodsid'].'',
                            'msg'=>'继续',
                        ),
                        array(
                            'url'=>'index.php?model=goods',
                            'msg'=>'返回',
                        )
                    );
    ...
    public function export_csvOp() {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?model=goods&fun=index');
