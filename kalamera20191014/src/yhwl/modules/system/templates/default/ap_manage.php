...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?model=adv&fun=get_ap_xml',
        colModel : [
            ...
            {display: '正在展示', name : 'ap_now_count', width : 100, sortable : false, align: 'left'},
            {display: '排序', name : 'ap_sort', width : 60, sortable : false, align: 'left'},
...
function fg_delete(id){
	...
	$.ajax({
        type: "GET",
        dataType: "json",
        url: "index.php?model=adv&fun=delete",
        data: "del_id="+id,
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?model=adv&fun=ap_add';
    }else if (name == 'delete') { 
...
{
   ajax_form('copy_adv', '<?php echo $lang['ap_get_js'];?>', 'index.php?model=adv&fun=ap_copy&id='+id);
}
</script> 
