...
class orderControl extends SystemControl{
    ...
    public function get_xmlOp(){
        ...
        foreach ($order_list as $order_id => $order_info) {
            ...
            $list['operation'] = "<a class=\"btn green\" href=\"index.php?model=order&fun=show_order&order_id={$order_info['order_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            ...
            if ($order_info['if_system_receive_pay']) {
                ...
                $operation_detail .= "<li><a href=\"index.php?model=order&fun=change_state&state_type=receive_pay&order_id={$order_info['order_id']}\">{$op_name}</a></li>";
            }
            if ($operation_detail) {
    ...
    public function export_step1Op(){
        ...
        $this->_get_condition($condition);
        $sort_fields = array('buyer_name','store_name','order_id','payment_code','order_state','order_amount','order_from','pay_sn','rcb_amount','pd_amount','payment_time','finnshed_time','evaluation_state','refund_amount','buyer_id','buyer_phone','store_id');
        if ($_POST['sortorder'] != '' && in_array($_POST['sortname'],$sort_fields)) {
            $order = $_POST['sortname'].' '.$_POST['sortorder'];
        } else {
            $order = 'order_id desc';
        ...
        // $where = 'gc_parent_id=2';
        // $jiugui= $model->table('goods_class')->field('gc_id,gc_name')->where($where)->select();
        // foreach ($jiugui as $key => $val) {
        //     $fenlei[] = $val['gc_id'];
        // }
        // echo "<pre>";
        // print_R($fenlei);die;
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?model=order&fun=index');
                Tpl::setDirquna('shop');
                Tpl::showpage('export.excel');
            }else{  //如果数量小，直接下载
                $data = $model_order->getOrderList($condition,'','*',$order,self::EXPORT_SIZE,array('order_common','order_goods'));
                $this->createExcel($data);
            }
        }else{  //下载
            $limit1 = ($_GET['curpage']-1) * self::EXPORT_SIZE;
            $limit2 = self::EXPORT_SIZE;
            $data = $model_order->getOrderList($condition,'','*',$order,"{$limit1},{$limit2}",array('order_common','order_goods'));
    ...
    private function createExcel($data = array()){
        ...
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'产品名称');
        ...
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'收货人手机号');
...
//        print_R($data);die;
        foreach ((array)$data as $k=>$order_info){
            ...
            $goods_name = '';
            $goods_num = '';
            foreach ((array)$order_info['extend_order_goods'] as $key=>$val){
                $goods_name .= $val['goods_name'].',';
                $goods_num .= $val['goods_num'].',';
            }
            $list['goods_name'] = rtrim($goods_name,',');
            $list['goods_num'] =  rtrim($goods_num,',');
            ...
            $list['order_state'] = $order_info['state_desc'];
            $list['buyer_phone'] = !empty($order_info['buyer_phone']) ? $order_info['buyer_phone'] : '';
            $list['pay_sn'] = empty($order_info['pay_sn']) ? '' : $order_info['pay_sn'];
            $list['payment_code'] = orderPaymentName($order_info['payment_code']);
            $list['payment_time'] = !empty($order_info['payment_time']) ? (intval(date('His',$order_info['payment_time'])) ? date('Y-m-d H:i:s',$order_info['payment_time']) : date('Y-m-d',$order_info['payment_time'])) : '';
            // $list['rcb_amount'] = ncPriceFormat($order_info['rcb_amount']);
