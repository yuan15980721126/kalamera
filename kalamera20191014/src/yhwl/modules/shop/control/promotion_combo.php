...
class promotion_comboControl extends SystemControl{
    private $links = array(
        array('url'=>'model=promotion_combo&fun=index','text'=>'商品列表'),
        array('url'=>'model=promotion_combo&fun=combo_quota_list','text'=>'套餐列表'),
        array('url'=>'model=promotion_combo&fun=combo_setting','text'=>'设置')
    );
    public function __construct(){
        ...
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>'index.php?model=promotion_combo&promotion_allow=1',
                    'msg'=>L('open'),
                )
            );
