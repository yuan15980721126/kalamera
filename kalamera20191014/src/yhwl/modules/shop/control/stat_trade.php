...
class stat_tradeControl extends SystemControl{
    private $links = array(
        array('url'=>'model=stat_trade&fun=income','lang'=>'stat_sale_income'),
        array('url'=>'model=stat_trade&fun=predeposit','lang'=>'stat_predeposit'),
        array('url'=>'model=stat_trade&fun=sale','lang'=>'stat_sale')
    );
    ...
    public function get_income_xmlOp(){
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                $out_array = array();
                $out_array['operation'] = '';
                //$out_array['operation'] = '<a class="btn green" href="index.php?model=stat_trade&fun=sale&search_type=month&searchmonth_month='.$_GET['search_month'].
            ...
            foreach ($list as $k => $v){
                ...
                switch ($out_array['order_state']){
                    ...
                    case ORDER_STATE_NEW:
                        ...
                        break;
                    case ORDER_STATE_PAY:
                        $out_array['order_state'] = 'To be shipped';
                        break;
                    case ORDER_STATE_SEND:
