...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?model=store&fun=get_xml',
        colModel : [
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?model=store&fun=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?model=store&fun=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
});
...
function fg_operations(name, bDiv) {
    if (name == 'haoshop_add') {
        window.location.href = 'index.php?model=store&fun=haoshop_add';
    }
}
