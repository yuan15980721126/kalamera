...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?model=ownshop&fun=get_xml',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?model=ownshop&fun=add';
    }
}

function fg_delete(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?model=ownshop&fun=del', {id:id}, function(data){
            if (data.state) {
