...
<div class="page">
  ...
  <div class="explanation" id="explanation">
    ...
    <ul>
      <li>买家提交申请，商家同意并经平台确认后，退款金额返还给买家账户。</li>
    </ul>
  </div>
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?model=refund&fun=get_manage_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?model=refund&fun=get_manage_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?model=refund&fun=get_manage_xml',
        colModel : [
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = $("#flexigrid").flexSimpleSearchQueryString()+'&fun=export_step1&refund_state=2&refund_id=' + id;
}
</script> 
