...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?model=snstrace&fun=get_xml',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?model=snstrace&fun=commentlist';
    }
}
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?model=snstrace&fun=tracedel', {id:id}, function(data){
            if (data.state) {
