...
<div class="alert mt10">
	<ul class="mt5">
		...
		<li>2、统计图展示了符合search条件的有效订单中的下单总金额和下单数量在时间段内的走势情况及与前一个时间段的趋势对比</li>
        <li>3、统计表显示了符合search条件的全部有效订单记录并可以点击“导出Excel”将订单记录导出为Excel文件</li>
    </ul>
</div>
<form method="get" action="index.php" target="_self">
  <table class="search-form">
    <input type="hidden" name="model" value="statistics_sale" />
    <input type="hidden" name="fun" value="sale" />
    <tr>
    	<td class="tr">
    		<div class="fr">
    		...
    		<div class="fr">&nbsp;
                  ...
                  <option value="<?php echo ORDER_STATE_PAY; ?>" <?php echo $_REQUEST['order_type']!='' && $_REQUEST['order_type']==ORDER_STATE_PAY?'selected':''; ?>>To be shipped</option>
...
<div class="alert alert-info mt10" style="clear:both;">
    ...
    <li>
    	<span class="w210 fl h30" style="display:block;">
    		<i title="店铺符合search条件的订单总金额" class="tip icon-question-sign"></i>
    		总下单金额：<strong><?php echo $output['statcount_arr']['orderamount'].$lang['currency_zh'];?></strong>
    	</span>
		<span class="w210 fl h30" style="display:block;">
			<i title="店铺符合search条件的订单数量" class="tip icon-question-sign"></i>
...
$(function(){
	...
	$("[name='searchweek_month']").change(function(){
		...
		$.getJSON('index.php?model=index&fun=getweekofmonth',{y:year,m:month},function(data){
	        if(data != null){
	...
	$('#statlist').load('index.php?model=statistics_sale&fun=salelist&t=<?php echo $output['searchtime'];?>&order_type=<?php echo $_REQUEST['order_type'];?>');
});
</script>