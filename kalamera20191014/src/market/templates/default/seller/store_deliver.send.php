...
<div class="wrap">
  ...
  <form name="deliver_form" method="POST" id="deliver_form" action="index.php?model=store_deliver&fun=send&order_id=<?php echo $_GET['order_id'];?>" onsubmit="ajaxpost('deliver_form', '', '', 'onerror');return false;">
    ...
    <table class="ncsc-default-table order deliver">
      <tbody>
        ...
        <tr>
          ...
          <td class="bdl bdr order-info w500" rowspan="<?php echo count($output['order_info']['extend_order_goods']);?>"><dl>
            ...
            <dl>
              ...
              <dd>
                <textarea name="deliver_explain" id="deliver_explain" cols="100" rows="2" class="w400 tip-t" title="<?php echo $lang['store_deliver_forget_tips'];?>"><?php echo $output['order_info']['extend_order_common']['deliver_explain'];?></textarea>
              </dd>
            </dl>
              <dl>
                  <a href="javascript:void(0)" id="store_deliver_forget_submit" class="ncbtn"><?php echo $lang['nc_common_button_save'];?></a></td>
              </dl>
        ...
        <tr>
          ...
          <a href="javascript:void(0)" nc_type="dialog" dialog_title="<?php echo $lang['store_deliver_buyer_adress'];?>" dialog_id="edit_buyer_address" uri="index.php?model=store_deliver&fun=buyer_address_edit&order_id=<?php echo $output['order_info']['order_id'];?>" dialog_width="550" class="ncbtn-mini fr"><i class="icon-edit"></i><?php echo $lang['nc_edit'];?></a></td>
        </tr>
        <?php } else { ?>
        <tr>
    ...
    <div class="deliver-sell-info"><strong class="fl"><?php echo $lang['store_deliver_my_daddress'].$lang['nc_colon'];?></strong>
      <a href="javascript:void(0);" onclick="ajax_form('modfiy_daddress', '<?php echo $lang['store_deliver_select_daddress'];?>', 'index.php?model=store_deliver&fun=send_address_select&order_id=<?php echo $output['order_info']['order_id'];?>', 640,0);" class="ncbtn-mini fr"><i class="icon-edit"></i><?php echo $lang['nc_edit'];?></a> <span id="seller_address_span">
...
$(function(){
    ...
    $('#my_address_add').click(function(){
		ajax_form('my_address_add', '<?php echo $lang['store_deliver_add_daddress'];?>' , 'index.php?model=store_deliver&fun=send_address_edit', 550,0);
    });

    $('#store_deliver_forget_submit').on('click',function(){
        var deliver_explain = $('#deliver_explain').val();
        if (deliver_explain == ''){
            showDialog('<?php echo $lang['store_deliver_bforget_pl'];?>', 'error','','','','','','','','',2);return false;
        }
        if(deliver_explain)
            $.post(
                "<?php echo urlShop('store_deliver', 'buyer_explain_save');?>",
                {
                ...
                .done(function(data) {
                    if(data == 'true') {
                        $('#deliver_explain').val(deliver_explain);
                        showSucc('修改成功');
                    } else {
                        showError('修改失败');
                    }
