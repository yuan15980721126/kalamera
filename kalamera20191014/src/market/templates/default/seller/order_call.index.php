...
<form method="get" action="index.php" target="_self">
  <table class="search-form">
    <input type="hidden" name="model" value="order_call" />
...
<span class="fr mr5"> <a href="index.php?model=store_order_print&order_id=<?php echo $order_id;?>" class="ncsc-btn-mini" target="_blank" title="打印发货单"/><i class="icon-print"></i>A5打印</a></span>
<span class="fr mr5"> <a href="index.php?model=store_order_print&fun=small&order_id=<?php echo $order_id;?>" class="ncsc-btn-mini" target="_blank" title="打印发货单"/><i class="icon-print"></i>小票打印</a></span>
 </th>
    </tr>
    <?php $i = 0;?>
    ...
    <tr>
      ...
      <td class="bdl bdr" rowspan="<?php echo $order['goods_count'];?>"><p><?php echo $order['state_desc']; ?>
        ...
        <!-- 订单查看 -->
        <p><a href="index.php?model=store_order&fun=show_order&order_id=<?php echo $order_id;?>" target="_blank"><?php echo $lang['store_order_view_order'];?></a></p>
        
        <!-- 物流跟踪 -->
        <p>
          <?php if ($order['if_deliver']) { ?>
          <a href='index.php?model=store_deliver&fun=search_deliver&order_sn=<?php echo $order['order_sn']; ?>'><?php echo $lang['store_order_show_deliver'];?></a>
          <?php } ?>
        </p>
...
function refresh()
{
	$.ajax({
		type: "GET",
		url: SHOP_SITE_URL+'/index.php?model=order_call&fun=getajaxorder',
		dataType:"json",
		async: true,
		success: function(data){
				...
				for(var i = 0; i < order_list.length; i++)
				{
					...
					for(var k = 0; k < order_list[i].goods_list.length; k++)
					{
						...
						if ((order_list[i].goods_count > 0 && k==0) || (order_list[i].goods_count) == 1 )
						{
							...
							//订单查看
							html_str += '	<p><a href="index.php?model=store_order&fun=show_order&order_id='+order_list[i].order_id+'" target="_blank">订单详情</a></p>';
								
							//物流跟踪
							html_str += '		<p>';
							if(order_list[i].if_deliver)
							{
								html_str += '<a href=\'index.php?model=store_deliver&fun=search_deliver&order_sn='+order_list[i].order_sn+'\'>查看物流</a>';
							}
							...
							if(order_list[i].if_send){
								html_str += '<p>';
								html_str += '<a class="ncsc-btn ncsc-btn-green mt10" href="index.php?model=store_deliver&fun=send&order_id='+order_list[i].order_id+'">';
			...
			{
				html_str += '<tr>';
				html_str += '  <td colspan="20" class="norecord"><div class="warning-option"><i class="icon-warning-sign"></i><span>No eligible data records</span></div></td>';
				html_str += '</tr>';	
			}
...
{
    if(length > 0){
		var url = SHOP_SITE_URL+'/index.php?model=order_call_print&fun=index';
...
{
	if(length > 0){
		var url = SHOP_SITE_URL+'/index.php?model=order_call_print&fun=small';
	    window.open(url,'_blank');
	}
}
...
function select_type(obj)
{
	...
	$.ajax({
		type: "GET",
		url: SHOP_SITE_URL+'/index.php?model=order_call&fun=select_type&type='+type,
		dataType:"json",
		async: true,
		success: function(data){
			if(data == 1)
