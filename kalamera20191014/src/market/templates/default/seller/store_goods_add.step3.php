...
<form method="post" id="goods_image" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_image'); } else { echo urlShop('store_goods_add', 'save_image');}?>">
  ...
  <div class="ncsc-form-goods-pic">
    <div class="container">
      ...
      <div class="ncsc-goodspic-list">
        ...
        <ul class="goods-pic-list" nctype="ul<?php echo $value['sp_value_id'];?>">
          ...
          <li class="ncsc-goodspic-upload">
            ...
            <div class="ncsc-upload-btn jsUploadImageBtn"><a href="javascript:void(0);"><span><input type="file" hidefocus="true" size="1" class="input-file" name="file_<?php echo $value['sp_value_id'] . $i;?>" id="file_<?php echo $value['sp_value_id'] . $i;?>"></span><p><i class="icon-upload-alt"></i>上传</p>
              </a></div>
        ...
        <div class="ncsc-select-album">
          <a class="ncbtn" href="index.php?model=store_album&fun=pic_list&item=goods_image&color_id=<?php echo $value['sp_value_id'];?>" nctype="select-<?php echo $value['sp_value_id'];?>"><i class="icon-picture"></i>从图片空间选择</a>
      ...
      <div class="ncsc-goodspic-list">
            ...
            <ul class="goods-pic-list">
                <?php for ($i = 0; $i < 10; $i++) {?>
                    <li class="ncsc-goodspic-upload" style="height:270px;">
                        <div class="upload-thumb">
                            <img src="<?php echo $output['files'][$i] ? '/data/upload/shop/common/file.png' : cthumb('abc.jpg', 240);?>" nctype="file2_<?php echo $i;?>">
                            <input type="hidden" name="files[<?php echo $i;?>][name]" value="<?php echo $output['files'][$i]['name'];?>" nctype="file2_<?php echo $i;?>">
                        </div>
                        <div class="show-default" nctype="file2_<?php echo $i;?>">
                            <p><i class="icon-ok-circle"></i>
                            </p><a href="javascript:void(0)" nctype="del" class="del" title="移除">X</a>
                        </div>
                        <div class="show-sort">排序：<input name="files[<?php echo $i;?>][sort]" type="text" class="text" value="<?php echo intval($output['files'][$i]['sort']);?>" size="2" maxlength="2">
                        </div>
                        <div class="ncsc-upload-btn jsUploadFileBtn">
                            <a href="javascript:void(0);">
                                <span><input type="file" hidefocus="true" size="1" class="input-file" name="file2_<?php echo $i;?>" id="file2_<?php echo $i;?>"></span>
                                <p><i class="icon-upload-alt"></i>上传</p>
                            </a>
                        </div>

                        <div class="show-text" style="position:absolute;bottom:10px;width:120px;left:10px;right:10px;">
                            描述：
                            <textarea style="width:110px; height:60px;" name="files[<?php echo $i?>][description]"><?php echo $output['files'][$i]['description']?></textarea>
                        </div>
        ...
        <div class="ncsc-goodspic-list">
            ...
            <ul class="goods-pic-list">
                <?php for ($i = 0; $i < 8; $i++) {?>
                    <li class="ncsc-goodspic-upload" style="height:170px;border: solid 1px #FFD700">

                        <div class="show-title">
                            排序：<input name="repair[<?php echo $i?>][sort]" type="text" class="text" value="<?php echo intval($output['repair'][$i]['sort']);?>" >
                        </div>
                        <div class="show-title">
                            延保描述：<input name="repair[<?php echo $i?>][description]" type="text" class="text" value="<?php echo $output['repair'][$i]['description'];?>">
                        </div>
                        <div class="show-title">
                            价格百分比：<input name="repair[<?php echo $i?>][percent]" type="text" class="text" value="<?php echo intval($output['repair'][$i]['percent']);?>">
                        </div>
    ...
    <div class="faq-img"></div>
        <h4>文件上传要求：</h4>
        <ul>
          <li>1. 请使用word\pdf等格式、单个文档大小不超过<?php echo intval(C('image_max_filesize'))/1024;?>M。</li>
          <li>4. 通过更改排序数字修改商品文档的排列显示顺序。</li>
          <li>6. 操作完成后请点下一步，否则无法在网站生效。</li>
        </ul>
  ...
  <div class="bottom tc hr32"><label class="submit-border"><input type="submit" class="submit" value="<?php if ($output['edit_goods_sign']) { echo 'submit'; } else { ?><?php echo $lang['store_goods_add_next'];?>，确认商品发布<?php }?>" /></label></div>
</form>
...
<script type="text/javascript" src="<?php echo SHOP_RESOURCE_SITE_URL;?>/js/store_goods_add.step3_2.js" charset="utf-8"></script>
<script>
