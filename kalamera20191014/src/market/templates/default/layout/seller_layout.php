  ...
  <div class="wrapper">
    <div class="ncsc-admin">
      ...
      <div class="ncsc-admin-function">
          <a href="<?php echo urlMember('member_security', 'auth',array('type'=>'modify_pwd'));?>" title="修改密码" target="_blank"><i class="icon-wrench"></i></a>
          <a href="<?php echo urlShop('seller_logout', 'logout');;?>" title="安全退出" style="float: right;"><i class="icon-signout"></i></a>
      </div>
    </div>
    ...
    <div class="index-search-container">
        ...
        <div class="sitemap-menu">
          ...
          <div id="quicklink_list" class="content">
            ...
            <dl <?php echo ($key == 'promotion' ? 'class="double"' : '');?>>
              ...
              <dd <?php if(!empty($output['seller_quicklink'])) {echo in_array($submenu_value['model'], $output['seller_quicklink'])?'class="selected"':'';}?>><i nctype="btn_add_quicklink" data-quicklink-model="<?php echo $submenu_value['model'];?>" class="icon-check" title="添加为常用功能菜单"></i><a href="index.php?model=<?php echo $submenu_value['model'];?>&fun=<?php echo $submenu_value['fun'];?>"> <?php echo $submenu_value['name'];?> </a></dd>
              <?php } ?>
              <?php } ?>
            </dl>
            <?php } ?>
      ...
      <div class="search-bar">
        <form method="get" target="_blank">
          ...
          <input type="text" nctype="search_text" name="keyword" placeholder="商城商品search" class="search-input-text">
    ...
    <nav class="ncsc-nav">
      <dl class="<?php echo $output['current_menu']['model'] == 'index'?'current':'';?>">
        <dt><a href="index.php?model=seller_center&fun=index">首页</a></dt>
        <dd class="arrow"></dd>
      </dl>
      <?php if(!empty($output['menu']) && is_array($output['menu'])) {?>
      <?php foreach($output['menu'] as $key => $menu_value) {?>
      <dl class="<?php echo $output['current_menu']['model'] == $key?'current':'';?>">
        <dt><a href="index.php?model=<?php echo $menu_value['child'][key($menu_value['child'])]['model'];?>&fun=<?php echo $menu_value['child'][key($menu_value['child'])]['fun'];?>"><?php echo $menu_value['name'];?></a></dt>
        <dd>
          <ul>
            <?php if(!empty($menu_value['child']) && is_array($menu_value['child'])) {?>
            <?php foreach($menu_value['child'] as $submenu_value) {?>
            <li> <a href="index.php?model=<?php echo $submenu_value['model'];?>&fun=<?php echo $submenu_value['fun'];?>"> <?php echo $submenu_value['name'];?> </a> </li>
            <?php } ?>
            <?php } ?>
          </ul>
        </dd>
    ...
    <div id="sidebar" class="sidebar">
      ...
      <div class="column-menu">
        <ul id="seller_center_left_menu">
          ...
          <li <?php echo $_GET['model'] == 'seller_center'?"id='quicklink_".$submenu_value['model']."'":'';?>class="<?php echo $submenu_value['model'] == $_GET['model']?'current':'';?>"> <a href="index.php?model=<?php echo $submenu_value['model'];?>&fun=<?php echo $submenu_value['fun'];?>"> <?php echo $submenu_value['name'];?> </a> </li>
          <?php } ?>
          <?php } else { ?>
          <?php if ($_GET['model'] == 'seller_center') { ?>
...
<div id="tbox ">
  <div class="btn hide" id="msg" style="display:none;"><a href="<?php echo urlShop('store_msg', 'index');?>"><i class="msg"><?php if ($output['store_msg_num'] > 0) { ?><em><?php echo $output['store_msg_num'];?></em><?php } ?></i>站内消息</a></div>
  <div class="btn hide" id="im" style="display:none;"><i class="im"><em id="new_msg" style="display:none;"></em></i><a href="javascript:void(0);">在线联系</a></div>
  <div class="btn hide" id="gotop" style="display:none;"><i class="top"></i><a href="javascript:void(0);">返回顶部</a></div>
</div>
</body>
</html>
