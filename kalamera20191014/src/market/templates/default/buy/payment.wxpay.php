...
<div class="ncc-main">
  <div class="ncc-title">
    ...
    <h5>订单详情内容可通过查看<a href="index.php?model=member_order" target="_blank">我的订单</a>进行核对处理。</h5>
  </div>
  ...
  <div class="wxpayment"> 
    ...
    <div class="pay-weixin">
      ...
      <div class="p-w-bd">
        <div class="p-w-box">
          <div class="pw-box-hd"> <img width="298" src="<?php echo SHOP_SITE_URL?>/index.php?model=payment&fun=qrcode&data=<?php echo urlencode($output['pay_url']);?>"> </div>
          <div class="pw-box-ft">
            <p>请使用微信扫一扫</p>
            <p>扫描二维码支付</p>
          </div>
...
function queryOrderState(){
    $.ajax({
        type: "GET",
        url: "<?php echo SHOP_SITE_URL?>/index.php?model=payment&fun=query_state&<?php echo $output['args'];?>&m="+Math.random(),
        data: "",
        dataType: "json",
        timeout: 4000,
        success: function(result) {
            if(result.state==1){
                if (result.type == 'r') {
                	window.location.href = '<?php echo SHOP_SITE_URL.'/index.php?model=member_order'?>';
                } else {
                	window.location.href = '<?php echo SHOP_SITE_URL.'/index.php?model=member_vr_order&fun=index'?>';
                }
            }
        }
    });
