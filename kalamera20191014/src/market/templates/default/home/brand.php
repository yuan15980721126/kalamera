...
<div class="brand">
      ...
      <div class="brand_img">
        <ul>
            ...
            <?php foreach($output['ap_list'] as $key=>$val){?>
            <li>
              <a href="<?php echo $val['adv_url']?>">
                <img src="<?php echo UPLOAD_SITE_URL."/".ATTACH_ADV."/".$val['default_content']?>">
              </a>  
            </li>
            <?php }?>
            <?php }?>
...
<!-- <div class="body" style="background:#0f0d1a url(<?php echo SHOP_TEMPLATES_URL;?>/images/hao-ht-brand.jpg) no-repeat center top;">
  ...
  <article class="m-recomBrand">
    <section class="rowOfFour clearfix">
      <?php foreach($output['brand_r'] as $key=>$brand_r){$i++; $i<9?>
      <?php if($i<9){?>
  ...
  <div class="nch-brand-class">
    <div class="nch-brand-class-tab">
      ...
      <ul class="tabs-nav">
        ...
        <li class="<?php if ($i == 1) { echo 'tabs-selected';} ?>"><a href="javascript:void(0);"><?php echo $brand['brand_class'];?></a><b class="line">/</b></li>
        <?php }?>
      </ul>
      <?php }?>
    </div>
...
$(".nch-barnd-list li").hover(function(){$(this).find(".nch-barnd-list  li a").animate({top:"-20px"},400,"swing")},function(){$(this).find(".nch-barnd-list li").stop(!0,!1).animate({top:"0px"},400,"swing")});
