$(function(){
		...
		$.get('index.php?model=index&fun=login', function(result){
		    if(result=='0'){
				...
				if($('#tracereply_'+data.txtid).css("display")=='none'){
					//加载评论列表
			        $("#tracereply_"+data.txtid).load('index.php?model=store_snshome&fun=commenttop&id='+data.txtid);
	...
	$("[nc_type='scomment_del']").live('click',function(){
		var obj = $(this);
		showDialog('Are you sure you want to delete this information？','confirm', '', function(){
			var data_str = $(obj).attr('data-param');
	        eval("data_str = "+data_str);
	        ajax_get_confirm('','index.php?model=store_snshome&fun=delcomment&scid='+data_str.scid+'&stid='+data_str.stid);
			return false;
		});
	});
		...
		if(form.valid()){
			...
			if(!isNaN(cookienum) && cookienum >= MAX_RECORDNUM){
			    if (seccode.css('display') == 'none') {
	                ...
	                seccode.find("[name='codeimage']").attr('src','index.php?model=seccode&fun=makecode&nchash='+nchash+'&t=' + Math.random());
	...
	$("[nc_type='sd_del']").live('click',function(){
        ...
        eval("data_str = "+data_str);
        var url = "index.php?model=store_snshome&fun=deltrace&id="+data_str.txtid;
		showDialog('Are you sure you want to delete this information？','confirm', '', function(){
			ajaxget(url);
			return false;
		});
