...
class buy_virtualControl extends BaseBuyControl {
    ...
    public function buy_step3Op() {
        ...
        //转向到商城支付页面
        redirect('index.php?model=buy_virtual&fun=pay&order_id='.$result['data']['order_id']);
    }
    ...
    public function payOp() {
        ...
        if ($order_id <= 0){
            showMessage('Order does not exist','index.php?model=member_vr_order','html','error');
        }
        ...
        if (empty($order_info) || !in_array($order_info['order_state'],array(ORDER_STATE_NEW,ORDER_STATE_PAY))) {
            showMessage('No order to pay was found','index.php?model=member_order','html','error');
        }
        ...
        if (empty($payment_list)) {
            showMessage('No suitable payment method has been found yet','index.php?model=member_vr_order','html','error');
        }
