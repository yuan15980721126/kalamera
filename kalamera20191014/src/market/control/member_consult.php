...
class member_consultControl extends BaseMemberControl {
    ...
    private function profile_menu($menu_type,$menu_key='',$array=array()) {
        ...
        switch ($menu_type) {
            case 'my_consult':
                $menu_array = array(
                1=>array('menu_key'=>'consult_list',    'menu_name'=>Language::get('nc_member_path_all_consult'),           'menu_url'=>'index.php?model=member_consult&fun=my_consult'),
                2=>array('menu_key'=>'to_reply',    'menu_name'=>Language::get('nc_member_path_unreplied_consult'),         'menu_url'=>'index.php?model=member_consult&fun=my_consult&type=to_reply'),
                3=>array('menu_key'=>'replied', 'menu_name'=>Language::get('nc_member_path_replied_consult'),           'menu_url'=>'index.php?model=member_consult&fun=my_consult&type=replied'));
                break;
        }
        if(!empty($array)) {
            $menu_array[] = $array;
