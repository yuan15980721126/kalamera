...
class store_deliverControl extends BaseSellerControl {
    ...
    public function sendOp(){
        ...
        //         $express_list = $model_express->getExpressListByID($_POST['shipping_express_id']);
        //         print_R($express_list);
        // die;
        if ($if_allow_send) {
            showMessage(Language::get('wrong_argument'),'','html','error');
        }
     ...
     * 发货备忘保存
     */
    public function buyer_explain_saveOp() {
        $model_order = Model('order');
        $data = array();
    ...
    public function search_deliverOp(){
        ...
        if (empty($order_info) || $order_info['shipping_code'] == '') {
            showMessage('No message found','','html','error');
        }
    ...
    public function waybill_printOp() {
        ...
        if(empty($store_waybill_info)) {
            showMessage('First bind the print template', urlShop('store_waybill', 'waybill_manage'), '', 'error'); //请首先绑定打印模板
        }

        $waybill_info = $model_waybill->getWaybillInfo(array('waybill_id' => $store_waybill_info['waybill_id']));
        if(empty($waybill_info)) {
            showMessage('First bind the print template', urlShop('store_waybill', 'waybill_manage'), '', 'error');
        }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'deliver':
                $menu_array = array(
                array('menu_key'=>'deliverno',          'menu_name'=>Language::get('nc_member_path_deliverno'), 'menu_url'=>'index.php?model=store_deliver&fun=index&state=deliverno'),
                array('menu_key'=>'delivering',         'menu_name'=>Language::get('nc_member_path_delivering'),    'menu_url'=>'index.php?model=store_deliver&fun=index&state=delivering'),
                array('menu_key'=>'delivered',      'menu_name'=>Language::get('nc_member_path_delivered'), 'menu_url'=>'index.php?model=store_deliver&fun=index&state=delivered'),
                );
                break;
            case 'search':
                $menu_array = array(
                1=>array('menu_key'=>'nodeliver',           'menu_name'=>Language::get('nc_member_path_deliverno'), 'menu_url'=>'index.php?model=store_deliver&fun=index&state=nodeliver'),
                2=>array('menu_key'=>'delivering',          'menu_name'=>Language::get('nc_member_path_delivering'),    'menu_url'=>'index.php?model=store_deliver&fun=index&state=delivering'),
                3=>array('menu_key'=>'delivered',       'menu_name'=>Language::get('nc_member_path_delivered'), 'menu_url'=>'index.php?model=store_deliver&fun=index&state=delivered'),
