...
class cartControl extends BaseBuyControl {
    ...
    public function __construct() {
        ...
        $op = isset($_GET['fun']) ? $_GET['fun'] : $_POST['op'];
    ...
    public function indexOp() {
        ...
        foreach ($cart_list as $cart) {
            ...
            $cart['goods_spec'] = str_replace('N;','',$cart['goods_spec']);
            $cart['goods_repair'] = $cart['goods_repair'] ? unserialize($cart['goods_repair']) : '';
            $store_cart_list[] = $cart;
        }
//        echo "<pre>";
//         print_R($store_cart_list);
        ...
        //推荐商品
        $goods_commend_list = $model_goods->getGoodsCommendList($cart['store_id'],5);
        ...
        //浏览过的商品
        $viewed_goods = Model('goods_browse')->getViewedGoodsList($_SESSION['member_id'], 5);
    ...
    public function del_allOp() {
        $cart_ids = explode(',', rtrim($_GET['cart_ids'],','));
        ...
        foreach ($cart_ids as $id) {
            ...
            if ($_SESSION['member_id']) {
                //登录状态下删除数据库内容
                $delete = $model_cart->delCart('db',array('cart_id'=>$id,'buyer_id'=>$_SESSION['member_id']));
            } else {
                //未登录时删除cookie的购物车信息
    ...
    public function addOp() {
        $model_goods = Model('goods');
        $logic_buy_1 = Logic('buy_1');
        $goods_repair_percent = $_GET['goods_repair_percent'] ? intval($_GET['goods_repair_percent']) : '';
        $goods_repair_description = $_GET['goods_repair_description'] ? $_GET['goods_repair_description'] : '';
        $goods_repair = $arrs = [];
        $goods_repair['percent'] = $goods_repair_percent;
        $goods_repair['description'] = $goods_repair_description;
//        print_r($_GET);
        if (is_numeric($_GET['goods_id'])) {
            ...
            $goods_repair['repair_price'] = $goods_repair['original_price']= ncPriceFormat($goods_info['goods_price'] *($goods_repair['percent']/100));
            $arrs = serialize($goods_repair);
            $goods_info['goods_repair'] = $arrs;
    ...
    public function updateOp() {
        ...
        $new_repair = 0;
        if($goods_repair['percent'] !='' && $goods_repair['repair_price'] !='0.00'){
            $new_repair = ncPriceFormat($goods_repair['original_price'] * $quantity);
            $goods_repair['repair_price'] = $new_repair;
            $new_repair_info = serialize($goods_repair);
        }else{
            $new_repair_info = $cart_info['goods_repair'];
        }
//           print_r($new_repair);
        ...
        $data['goods_repair'] = $new_repair_info;
        ...
        if ($update) {
            ...
            $return['goods_price'] = $goods_info['goods_price'];
            $return['repair_price'] = $new_repair;
