...
class member_orderControl extends BaseMemberControl {
    ...
    public function indexOp() {
        ...
        foreach ($order_list as $order_id => $order_info) {
            ...
            $order_info['state_desc'] = orderStateEn($order_info);


            $order_group_list[$order_info['pay_sn']]['order_list'][] = $order_info;
        }
//         echo "<pre>";
//         print_R($order_group_list);
        $nav_link_list = array(
            array(
                'title'=>'My Account',
        ...
        $order_tip['order_pay_count'] = $model_order->getOrderCountByID('buyer',$_SESSION['member_id'],'PayCount');
        $order_tip['order_state_success'] = $model_order->getOrderCountByID('buyer',$_SESSION['member_id'],'EvalCount');
    ...
    public function get_expressOp(){
        ...
        if(strlen($_GET['t']) == 10){
            ...
            if (empty($content)) exit(json_encode(false));
    ...
    public function show_orderOp() {
        ...
        $result['data']['order_info']['state_desc'] = orderStateEn($result['data']['order_info']);
//        echo "<pre>";
//        print_R($result['data']['order_info']);
    ...
    private function _order_cancel($order_info, $post) {
        if (!chksubmit()) {
            ...
            if (!$if_allow) {
                return callback(false,'Unauthorized operation');
            }
            if (TIMESTAMP - 86400 < $order_info['api_pay_time']) {
                $_hour = ceil(($order_info['api_pay_time']+86400-TIMESTAMP)/3600);
                return callback(false,'The order was attempted to be paid on a third-party payment platform and could not be cancelled until' .$_hour.'hours later');
            }
    ...
    private function profile_menu($menu_key='') {
        ...
        $menu_array = array(
            array('menu_key'=>'member_order','menu_name'=>Language::get('nc_member_path_order_list'), 'menu_url'=>'index.php?model=member_order'),
//             array('menu_key'=>'member_order_recycle','menu_name'=>'回收站', 'menu_url'=>'index.php?model=member_order&recycle=1'),
        );
