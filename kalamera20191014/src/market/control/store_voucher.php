...
class store_voucherControl extends BaseSellerControl{
    ...
    public function voucher_exportOp(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){//显示下载链接
                ...
                Tpl::output('murl','index.php?model=store_voucher&fun=voucher_export');
    ...
    public function quotaaddOp(){
        if (chksubmit()){
            ...
            if($reault){
                showDialog(Language::get('voucher_apply_buy_succ'),'index.php?model=store_voucher&fun=quotalist','succ');
            } else {
                showDialog(Language::get('nc_common_op_fail'),'index.php?model=store_voucher&fun=quotalist');
            }
        }else {
            //输出导航
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(Language::get('nc_common_save_succ'),'index.php?model=store_voucher&fun=templatelist','succ');
                }else{
                    showDialog(Language::get('nc_common_save_fail'),'index.php?model=store_voucher&fun=templatelist','error');
                }
            }
        }else{
            //店铺分类
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(Language::get('nc_common_op_succ'),'index.php?model=store_voucher&fun=templatelist','succ');
                }else{
                    showDialog(Language::get('nc_common_op_fail'),'index.php?model=store_voucher&fun=templatelist','error');
                }
            }
        }else{
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'voucher':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'), 'menu_url'=>'index.php?model=store_voucher&fun=templatelist'),
                );
                break;
            case 'quota_add':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'), 'menu_url'=>'index.php?model=store_voucher&fun=templatelist'),
                4=>array('menu_key'=>'quotaadd','menu_name'=>Language::get('voucher_applyadd'), 'menu_url'=>'index.php?model=store_voucher&fun=quotaadd')
                );
                break;
            case 'template':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'), 'menu_url'=>'index.php?model=store_voucher&fun=templatelist'),
                2=>array('menu_key'=>'templateadd','menu_name'=>Language::get('voucher_templateadd'),   'menu_url'=>'index.php?model=store_voucher&fun=templateadd'),
                );
                break;
            case 'templateedit':
                $menu_array = array(
