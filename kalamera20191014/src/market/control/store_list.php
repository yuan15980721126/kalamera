...
class store_listControl extends BaseHomeControl {
	...
	public function indexOp(){
		...
		if (intval($_GET['cate_id']) > 0){
			$nav_link[1]['link'] = 'index.php?model=shop_search';
			...
			if ($nav['sc_parent_id'] > 0){
				...
				$nav_link[] = array(
					'title'=>$tmp['sc_name'],
					'link'=>"index.php?model=store_list&cate_id=".$nav['sc_parent_id']
				);
			}
			//存入当前级
			$nav_link[] = array(
		...
		Tpl::output('purl', urlShop($purl['model'], $purl['op'], $purl['param']));
