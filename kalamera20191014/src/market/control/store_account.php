...
class store_accountControl extends BaseSellerControl {
    ...
    public function account_addOp() {
        ...
        if (empty($seller_group_list)) {
            showMessage('Please set up account group', urlShop('store_account_group', 'group_add'), '', 'error'); //请先建立账号组
        }
    ...
    public function account_saveOp() {
        ...
        if(!$member_info) {
            showDialog('User authentication failed', 'reload', 'error');
        }

        $seller_name = $_POST['seller_name'];
        if($this->_is_seller_name_exist($seller_name)) {
            showDialog('Account already exists', 'reload', 'error');
        }
