...
class paymentLogic {
    ...
    public function getRealOrderInfo($pay_sn, $member_id = null) {
        ...
        $order_pay_info['order_id'] = $order_pay_info['order_id'];
        ...
        //商品列表信息
        $where['order_id'] = $order_list[0]['order_id'];
        $goods_list = $model_order->getOrderGoodsList($where);

        //订单收货信息
        $shipping = $model_order->getOrderCommonInfo($where);

        /*
        //BEGIN 补充字段，区域名称，代码 -- 主用于paypal账单地址替换 -- by siuloong 20190426
        $reciver_city = Model('area')->getAreaInfo(array('area_id' => $shipping['reciver_city_id']));
        $shipping['area_name'] = $reciver_city['area_name']; //城市名
        $reciver_state = Model('area')->getAreaInfo(array('area_id' => $shipping['reciver_province_id'])); //州
        $shipping['area_code'] = $reciver_state['area_code']; //州代码
        //END 补充字段，区域名称，代码
        */
        ...
        $order_pay_info['goods_list'] = $goods_list;
        $order_pay_info['shipping'] = $shipping;
    ...
    public function getVrOrderInfo($order_sn, $member_id = null) {
        ...
        if(empty($order_info)){
            return callback(false,'Order does not exist');
        }
