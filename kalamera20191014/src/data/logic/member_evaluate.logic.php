...
class member_evaluateLogic {
     ...
     * @return Ambigous <PHPUnit_Framework_Constraint_Callback, multitype:unknown >
     */
    public function saveone($date, $order_info, $store_info, $order_goods, $member_id, $member_name) {
        $model_order = Model('order');
        ...
        if(count($order_goods) > 0) {
            //如果未评分，默认为5分
            $evaluate_score = intval($date['goods'][$order_goods['rec_id']]['score']);
            if ($evaluate_score <= 0 || $evaluate_score > 5) {
                $evaluate_score = 5;
            }
            //默认评语
            $evaluate_comment = $date['goods'][$order_goods['rec_id']]['comment'];
            if (empty($evaluate_comment)) {
                $evaluate_comment = 'Not bad';
            }
            $geval_image = '';
            if (isset($date['goods'][$order_goods['rec_id']]['evaluate_image']) && is_array($date['goods'][$order_goods['rec_id']]['evaluate_image'])) {
                foreach ($date['goods'][$order_goods['rec_id']]['evaluate_image'] as $val) {
                    if (!empty($val)) {
                        $geval_image .= $val . ',';
                    }
                }
            ...
            $evaluate_goods_info['geval_ordergoodsid'] = $order_goods['rec_id'];
            $evaluate_goods_info['geval_goodsid'] = $order_goods['goods_id'];
            $evaluate_goods_info['geval_goodsname'] = $order_goods['goods_name'];
            $evaluate_goods_info['geval_goodsprice'] = $order_goods['goods_price'];
            $evaluate_goods_info['geval_goodsimage'] = $order_goods['goods_image'];
            $evaluate_goods_info['geval_scores'] = $evaluate_score;
            $evaluate_goods_info['geval_content'] = $evaluate_comment;
            $evaluate_goods_info['geval_isanonymous'] = $date['goods'][$order_goods['rec_id']]['anony'] ? 1 : 0;
            ...
            //print_R($evaluate_goods_info);
            //die;
            $goodsid_array = $order_goods['goods_id'];

        }
        //print_R($evaluate_goods_array);die;

        $model_evaluate_goods->addEvaluateGoods($evaluate_goods_info, $goodsid_array);
        //添加店铺评价
        ...
        $condition = array();
        $condition['geval_orderid'] = $order_info['order_id'];
        //print_R($condition);
        $evaluate_info = $model_evaluate_goods->getEvaluateGoodsList($condition);
        foreach ($evaluate_info as $evaluate){//查找该订单号下所有已评价的商品
            $is_evaluate[] = $evaluate['geval_goodsid'];
        }

        $is_same =array_diff($date['order_goods'],$is_evaluate);

        if(count($is_same) == 0){//当评价完该订单所有商品才更改订单状态（判断订单商品和评价商品表的商品id是否一样）
    ...
    public function save($date, $order_info, $store_info, $order_goods, $member_id, $member_name) {
        ...
        foreach ($order_goods as $value){
            ...
            $geval_image = rtrim($geval_image, ',');
            $common = Model('goods')->getGoodsInfo(array('goods_id'=>$value['goods_id']),'goods_commonid');
            ...
            $evaluate_goods_info['geval_goodscommonid'] = $common['goods_commonid'];;//新增商品公共id
    ...
    public function saveVr($date, $order_info, $store_info, $member_id, $member_name) {
        ...
        $common = Model('goods')->getGoodsInfo(array('goods_id'=>$order_info['goods_id']),'goods_commonid');
