...
class goodsModel extends Model{
    ...
    public function getGoodsDetail($goods_id) {
        ...
        $desc_id = $this->table('goods')->field("goods_id")->where("goods_commonid='$goods_info[goods_commonid]'")->select();
        if($desc_id){
            foreach ($desc_id as $key => $value) {
                $child_spec = $this->table('goods')->field("goods_spec")->where("goods_id='$value[goods_id]'")->find();
                if($child_spec){
                    $child[$value['goods_id']] = unserialize($child_spec['goods_spec']);
                }
            }
        }
        if($child){
