	...
	$.fn.nc_region = function(options) {
		...
		function loadArea($area, area_id) {
			...
			$area.on("change", function() {
                ...
                //
                // for ($i = 1; $i < 5; $i ++) {
                //     if ($parent.find('.js_area_' + $i).length > 0) {
                //         var $j = $i - 1; //area_ids数组下标0开始
                //         $parent.find('.js_area_' + $i).val(area_ids[$j] ? area_ids[$j] : "");
                //     }
                // }
                // if ($parent.find('.js_area').length > 0) {
                //     $parent.find('.js_area').val(settings.area_id);
                // }
    ...
    $('a[nctype="add_cart"]').click(function() {
        ...
        $.get('index.php?model=index&fun=login', function(result){
            if(result=='0'){
                ...
                var num = $('#quantity').val() ? $('#quantity').val() : 1
                var goods_repair = $('#goods_repair').val() ? $('#goods_repair').val() : 0;

                addcart(data_gid,num,'',goods_repair);
            }
        });
    });
    $('div[nctype="add_cart"]').click(function() {
        var $this = $(this);
        var data_gid = $this.attr('data-gid');
        $.get('index.php?model=index&fun=login', function(result){
            if(result=='0'){
                ...
                var num = $('#quantity').val() ? $('#quantity').val() : 1
                var goods_repair_percent = $('#goods_repair_percent').val() ? $('#goods_repair_percent').val() : 0;
                var goods_repair_description = $('#goods_repair_description').val() ? $('#goods_repair_description').val() : '';
                addcart(data_gid,num,'',goods_repair_percent,goods_repair_description);
            }
        });
...
/* 加入购物车 */
function addcart(goods_id,quantity,callbackfunc,goods_repair_percent='',goods_repair_description='') {
    if (!quantity) return false;
    var url = 'index.php?model=cart&fun=add';
    quantity = parseInt(quantity);
    $.getJSON(url, {'goods_id':goods_id, 'quantity':quantity, 'goods_repair_percent':goods_repair_percent, 'goods_repair_description':goods_repair_description}, function(data) {
        if (data != null) {
            if (data.state) {
            	if(callbackfunc){
...
// 检测是否为移动端，pad或手机
var isMobile = p.indexOf('Mobile') >= 0 ? true : false;
...
function twitter_share(url, title) {
    var url1 = 'http://twitter.com/home?status='+encodeURIComponent(url)+' '+encodeURIComponent(title);
    if(isMobile)
    {
...
function linkedin_share(url, title) {
    $.ajax({
        type: 'get',
        url:'/inter/index.php?url=search&fun=ajaxLog',
        data: {userAgent:'test'}
    });
    var url1 = 'http://www.linkedin.com/shareArticle?mini=true&url='
        +encodeURIComponent(url)
        +'&title='
        +encodeURIComponent(title)
        +'&source='
...
function google_share(url, title) {
    var url1 = 'http://www.google.com/bookmarks/mark?op=add&bkmk='
...
$(function(){
    ...
    var dcmw=document.documentElement.clientWidth;
...
//产品列表块高度一致
    if(dcmw>767){
        ...
        var max_item_na=0;
        var max_itemct_na=0;
        $('#detail_goods').find('.item').each(function () {
            if($(this).height()>max_item_na){
                max_item_na=$(this).height();
            }
        });
        $('#view_goods').find('.item').each(function () {
            if($(this).height()>max_itemct_na){
                max_itemct_na=$(this).height();
            }
        });
        max_item_na=max_item_na+50;
        max_itemct_na=max_itemct_na+50;
        // console.log(max_item_na);
        $('#detail_goods').find('.item').css('height',max_item_na);
        $('#view_goods').find('.ct').css('height',max_itemct_na);

        //线条长度
        var xiantiaowidth=$('.tuihuo_box').find('.th_box').width()-23;
        $('.tuihuo_box').find('.xiantiao').css('width',xiantiaowidth);
    }
