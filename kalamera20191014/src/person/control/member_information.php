...
class member_informationControl extends BaseMemberControl {
    ...
    public function memberOp() {
        ...
        if (chksubmit()){
            ...
            $member_array['member_truename']    = $truename;
        ...
        Tpl::output('menu_sign_url','index.php?model=member_information&fun=member');
...
public function uploadOp() {
		if (!chksubmit()){
			redirect('index.php?model=member_information&fun=avatar');
		}
        ...
        if (!empty($_FILES['pic']['tmp_name'])){
            ...
            showMessage('Upload failed, please try to change the picture format or small picture','','html','error');//上传失败，请尝试更换图片格式或小图片
        }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'member':
                $menu_array = array(
                1=>array('menu_key'=>'member',  'menu_name'=>Language::get('home_member_base_infomation'),'menu_url'=>'index.php?model=member_information&fun=member'),
                2=>array('menu_key'=>'more',    'menu_name'=>Language::get('home_member_more'),'menu_url'=>'index.php?model=member_information&fun=more'),
                5=>array('menu_key'=>'avatar',  'menu_name'=>Language::get('home_member_modify_avatar'),'menu_url'=>'index.php?model=member_information&fun=avatar'));
                break;
        }
