...
class member_sharemanageControl extends BaseMemberControl {
    public function __construct(){
        ...
        //判断系统是否开启站外分享功能
        if (C('share_isuse') != 1){
            showMessage(Language::get('sharebind_unused'),'index.php?model=member_information&fun=member','html','error');
        }
    }
    /**
     * 分享绑定列表
    ...
    public function unbindOp(){
        $type = $_GET['type'];
        if (empty($type)){
            showDialog(Language::get('wrong_argument'),'index.php?model=member_sharemanage','error');
        }
        $model = Model('sns_binding');
    ...
    public function share_qqzoneOp(){
        ...
        if (C('share_qqzone_isuse') != 1){
            showMessage(Language::get('sharebind_unused'),'index.php?model=member_sharemanage','html','error');
        }
