...
class member_messageControl extends BaseMemberControl {
    ...
    public function systemmsgOp(){
        ...
        // $page = 2;
        $page->setStyle('admin');
        $message_array	= $model_message->listMessage(array('from_member_id'=>'0','message_type'=>'1','to_member_id'=>$_SESSION['member_id'],'del_member_id'=>$_SESSION['member_id']),$page);
        // echo "<pre>";
        ...
        if (!empty($message_array) && is_array($message_array)){
            foreach ($message_array as $k=>$v){
                ...
                $model_message->updateSystemMessage(array('read_member_id'=>$tmp_readid_str),array('message_id'=>"{$message_id}"));
            }
        }
        Tpl::output('show_page',$page->show());
    ...
    public function dropbatchmsgOp() {
        ...
        if(!in_array($drop_type,array('msg_system','msg_seller')) || empty($message_id)){
            if($json == 'json' ){
                exit(json_encode(array('state'=>faild,'msg'=>Language::get('home_message_delete_request_wrong'))));
            }
        ...
        if ($drop_state){
            ...
            setNcCookie($cookie_name,$countnum,2*3600);//保存2小时
            if($json == 'json' ){
                exit(json_encode(array('state'=>true,'msg'=>Language::get('home_message_delete_success'))));
            }
            showDialog(Language::get('home_message_delete_success'),'reload','succ');
        }else {
            if($json == 'json' ){
                exit(json_encode(array('state'=>faild,'msg'=>Language::get('home_message_delete_fail'))));
            }
            showDialog(Language::get('home_message_delete_fail'),'','error');
        }
    }
    ...
    private function showReceivedNewNum() {
        ...
        $condition['article_position'] = array('in', ARTICLE_POSIT_ALL.','.ARTICLE_POSIT_BUYER );

        $article  = $model_message->getCount($condition);
        // print_R($article);
        Tpl::output('newarticle',$article);
    }
