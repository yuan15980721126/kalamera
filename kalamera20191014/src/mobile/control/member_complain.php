...
class member_complainControl extends mobileHomeControl{
    //定义投诉状态常量
    const STATE_NEW = 10;
    ...
    public function indexOp() {
        ...
        $condition['accuser_id'] = $this->member_id;
        // var_dump($this->member_id);
        if($_GET['select_complain_state']){
            $condition['complain_state'] = $_GET['select_complain_state'];   
        }
        ...
        if(!empty($goods_list)){
            foreach ($list as $key => $value) {
                if($value['complain_datetime']){
                    $list[$key]['complain_datetime'] = date('Y-m-d H:i:s',$value['complain_datetime']);
                }
                if($value['complain_handle_datetime']){
                    $list[$key]['complain_handle_datetime'] = date('Y-m-d H:i:s',$value['complain_handle_datetime']);
                }
                if($value['final_handle_datetime']){
                    $list[$key]['final_handle_datetime'] = date('Y-m-d H:i:s',$value['final_handle_datetime']);
                }
                if($value['appeal_datetime']){
                    $list[$key]['appeal_datetime'] = date('Y-m-d H:i:s',$value['appeal_datetime']);
                }
                foreach ($goods_list as $k => $v) {
                    if($value['order_goods_id'] == $k){
                        $list[$key]['goods_detail'] = $v['goods_name'];
                        $list[$key]['goods_id'] = $v['goods_id'];
                    }
                    
                }
            }
            $arr =array('list'=>$list,'goods_list'=>$goods_list);
            output_data($arr);
        }else{
            output_data(1);
        }
    ...
    public function complain_newOp() {
        ...
        if ($order_id < 1 || $goods_id < 1) {//参数验证
            showMessage(Language::get('wrong_argument'),'index.php?model=member_order&fun=index','html','error');
        }
        $condition = array();
        ...
        $arr =array('list'=>$complain_subject_list);
        output_data($arr);
    ...
    public function complain_saveOp() {
        ...
        // print_R($_POST);die;
        $input = array();
        $input['order_id'] = intval($_POST['order_id']);
        $input['order_goods_id'] = intval($_POST['order_goods_id']);
        $condition = array();
        $condition['buyer_id'] = $this->member_id;
        ...
        $input['complain_subject_id'] = intval($_POST['reason_id']);
        ...
        $input['complain_pic1'] = trim($_POST['input_complain_pic1']);
        $input['complain_pic2'] = trim($_POST['input_complain_pic2']);
        $input['complain_pic3'] = trim($_POST['input_complain_pic3']);
        // print_R($input);die;
        $model_complain = Model('complain');
    ...
    public function apply_handleOp() {
        ...
        showMessage(Language::get('handle_submit_success'),'index.php?model=member_complain');
    }
    ...
    public function upload_picOp() {
        $upload = new UploadFile();
        $dir = ATTACH_PATH.DS.'complain'.DS;
        $upload->set('default_dir',$dir);
        $upload->set('allow_type',array('jpg','jpeg','gif','png'));
        $result = 0;
        if (!empty($_FILES['input_complain_pic1']['name'])){
            $result = $upload->upfile('input_complain_pic1');
        }
        if (!empty($_FILES['input_complain_pic2']['name'])){
            $result = $upload->upfile('input_complain_pic2');
        }
        if (!empty($_FILES['input_complain_pic3']['name'])){
            $result = $upload->upfile('input_complain_pic3');
        }
        if ($result){
            $file_name = $upload->file_name;
            $pic = UPLOAD_SITE_URL.'/'.ATTACH_PATH.'/complain/'.$file_name;
             // $pic = $file_name;
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            array('menu_key'=>'complain_accuser_list','menu_name'=>Language::get('complain_manage_title'),'menu_url'=>'index.php?model=member_complain')
        );
