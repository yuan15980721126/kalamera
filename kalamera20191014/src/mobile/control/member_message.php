...
class member_messageControl extends mobileMemberControl {
    ...
    public function systemmsgOp(){
        ...
        // $page->setStyle('admin');
        $page = 5;
        $message_array	= $model_message->listMessage(array('from_member_id'=>'0','message_type'=>'1','to_member_id'=>$this->member_info['member_id'],'del_member_id'=>$this->member_info['member_id']),$page);
        if (!empty($message_array) && is_array($message_array)){
            foreach ($message_array as $k=>$v){
                ...
                if(strstr($v['message_body'],'代金券')){
                    $v['message_title'] = '代金券已使用';
                    $start = strpos($v['message_body'],"<a href");
                    $replace = substr($v['message_body'],$start);
                    $v['message_jiandan'] = str_replace($replace,'', $v['message_body']);
                }else if(strstr($v['message_body'],'已经出库')){
                    $v['message_title'] = '订单已经出库';
                    $start = strpos($v['message_body'],"<a href");
                    $replace = substr($v['message_body'],$start);
                    $v['message_jiandan'] = str_replace($replace,'', $v['message_body']);
                }else if(strstr($v['message_body'],'款项已经收到')){
                    $v['message_title'] = '留意出库通知';
                    $start = strpos($v['message_body'],"<a href");
                    $replace = substr($v['message_body'],$start);
                    $v['message_jiandan'] = str_replace($replace,'', $v['message_body']);
                }else if(strstr($v['message_body'],'已经回复')){
                    $v['message_title'] = '回复消息';
                    $start = strpos($v['message_body'],"<a href");
                    $replace = substr($v['message_body'],$start);
                    $v['message_jiandan'] = str_replace($replace,'', $v['message_body']);
                }else if(strstr($v['message_body'],'退款退货')){
                    $v['message_title'] = '退款退货通知';
                    $start = strpos($v['message_body'],"<a href");
                    $replace = substr($v['message_body'],$start);
                    $v['message_jiandan'] = str_replace($replace,'', $v['message_body']);
                }
                ...
                $v['message_update_time'] = date("Y-m-d H:i:s",$v['message_update_time']);       
                $message_array[$k]	= $v;
            }
        }
        // echo "<pre>";
        ...
        Tpl::output('show_page', $model_message->showpage());
    
        // Tpl::output('drop_type','msg_system');
        // $this->profile_menu('system');
        // Tpl::showpage('member_message.box');

        output_data(array('message_info' => $message_array));
    }
    ...
    public function savemsgOp() {
        ...
        if (chksubmit()) {
            ...
            if($_GET['type'] == 'sns_board'){
                ...
                showDialog(Language::get('home_message_send_success'),'index.php?model=member_message&fun=privatemsg','succ');
            }
        }
    }
    /**
    ...
    public function savereplyOp() {
        ...
        if (!$isallowsend){
            if($_GET['inajax'] == 1){
                ...
                showMessage(Language::get('home_message_noallowsend'),'index.php?model=member_message&fun=message','html','error');
            }
        }
        if ($_POST['form_submit'] == 'ok') {
            $message_id = intval($_POST["message_id"]);
            if ($message_id <=0){
                showMessage(Language::get('wrong_argument'),'index.php?model=member_message&fun=message','html','error');
            }
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
            ...
            if($_GET['inajax'] == 1){
                ...
                showMessage(Language::get('home_message_send_success'),'index.php?model=member_message&fun=privatemsg');
            }
        }else {
            if($_GET['inajax'] == 1){
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            array('menu_key'=>'message', 'menu_name'=>Language::get('home_message_received_message'),'menu_url'=>'index.php?model=member_message&fun=message'),
            array('menu_key'=>'private', 'menu_name'=>Language::get('home_message_private_message'), 'menu_url'=>'index.php?model=member_message&fun=privatemsg'),
            array('menu_key'=>'system',  'menu_name'=>Language::get('home_message_system_message'),  'menu_url'=>'index.php?model=member_message&fun=systemmsg'),
            array('menu_key'=>'ggmsg',  'menu_name'=>'系统公告',  'menu_url'=>'index.php?model=member_message&fun=ggmsg'),
            array('menu_key'=>'close',   'menu_name'=>Language::get('home_message_close'),   'menu_url'=>'index.php?model=member_message&fun=personalmsg'),
            array('menu_key'=>'setting', 'menu_name'=>'接收设置', 'menu_url'=>urlMember('member_message', 'setting'))
        );
        if($menu_key == 'sendmsg') {
            $menu_array[] = array('menu_key'=>'sendmsg','menu_name'=>Language::get('home_message_send_message'),'menu_url'=>'index.php?model=member_message&fun=sendmsg');
