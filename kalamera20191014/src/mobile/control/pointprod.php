...
class pointprodControl extends mobileMemberControl{
    public function __construct() {
        ...
        if (C('pointprod_isuse') != 1){
            ...
            output_error(L('pointprod_unavailable'));
        }
    ...
    public function plistOp(){
        ...
        if (!$arr) {
            ...
            $arr = array('pointprod_list' => $pointprod_list, 'member_info' => $this->member_info);
            // 写入缓存
            wkcache('point_goods', $arr);
        }
        output_data($arr);
    ...
    public function pinfoOp() {
        ...
        $prodinfo['pgoods_starttime'] = date('Y-m-d H:i:s',$prodinfo['pgoods_starttime']);
        $prodinfo['pgoods_endtime'] = date('Y-m-d H:i:s',$prodinfo['pgoods_endtime']);

        if (empty($prodinfo)){
            // showDialog(L('pointprod_record_error'),urlShop('pointprod','plist'),'error');
            output_error(L('pointprod_record_error'));
        }
        // echo "<pre>";
        // print_R($prodinfo);
        ...
        $arr =array('prodinfo'=>$prodinfo,'orderprod_list'=>$orderprod_list,'recommend_pointsprod'=>$recommend_pointsprod);
        output_data($arr);
        $seo_param = array();
