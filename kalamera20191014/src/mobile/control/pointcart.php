...
class pointcartControl extends mobileMemberControl {
    public function __construct() {
        ...
        if (C('pointprod_isuse') != 1){
            ...
            output_error(L('pointcart_unavailable'));
        }
    ...
    public function addOp() {
        ...
        if (!$data['state']){
            switch ($data['error']){
                case 'ParameterError':
                    // echo json_encode(array('done'=>false,'msg'=>$data['msg'],'url'=>'index.php?model=pointprod&fun=plist')); die;
                // output_error('参数错误');
    ...
    public function step1Op(){
        ...
        if (!$data['state']){
            // showDialog($data['msg'],'index.php?model=pointprod','error');
            output_error('系统错误');
            // output_data(1);
        }
        // $logic_buy = logic('buy');
        ...
        $default = Model('address')->getDefaultAddressInfo(array('member_id'=>$this->member_info['member_id']));
        ...
        //     }
        // }
        // $address_list['address_api'] = $data_area ? $data_area : '';
        // $arr = array('address_list' => $address_list, 'default' => $default);
        // output_data($default);
        // Tpl::output('address_list',$address_list);
        // Tpl::output('address_info',$address_list[0]);
    ...
    public function step2Op() {
        ...
        if (!$data['state']){
            // showDialog($data['msg'],'index.php?model=pointcart','error');
             output_error('购物车信息错误');
        }
        ...
        unset($data);
        $arr = array('pointprod_arr' => $pointprod_arr, 'default' => $default);
        output_data($arr);
        ...
        output_data(1);
        // @header("Location:index.php?model=pointcart&fun=step3&order_id=".$order_id);
    }
    /**
     * 流程第三步
     */
    public function step3Op($order_arr=array()) {
        ...
        if (!$data['state']){
            output_error($data['msg']);
            // showDialog($data['msg'],'index.php?model=pointcart&fun=step1','error');
        }
