 ...
 */
function orderStateEn($order_info) {
    switch ($order_info['order_state']) {
        ...
        case ORDER_STATE_NEW:
            if ($order_info['chain_code']) {
                $order_state = 'Shop Payment Self-Payment';
            } else {
        ...
        case ORDER_STATE_PAY:
            if ($order_info['chain_code']) {
                $order_state = 'Pending mention';
            } else {
                $order_state = 'To be shipped';
            }
            break;
        case ORDER_STATE_SEND:
            $order_state = 'Goods to be received';
            break;
        case ORDER_STATE_SUCCESS:
            $order_state = 'Transaction completed';
            break;
    }
    return $order_state;
}
 ...
 * @return string 描述输出
 */
function orderGoodsEnType($goods_type) {
    return str_replace(array(
        '1','2','3','4','5',
        '8', '9',
    ), array(
        '','Rush to buy','Discount','Discount suit','Gift',
        '', 'Repurchase',
    ), $goods_type);
}
/**
 ...
 * @return string
 */
function pcthumb($goods_images = array(), $type = ''){
    if($goods_images){
        if(!is_array($goods_images)){
            $goods = json_decode($goods_images,true);
            if(!empty($goods['levelD']) && !strstr($goods['levelD'] ,'default')){
                return $goods['levelD'];
            }elseif(!empty($goods['levelC']) && !strstr($goods['levelC'] ,'default')){
                return $goods['levelC'];
            }elseif(!empty($goods['levelB']) && !strstr($goods['levelB'] ,'default')){
                return $goods['levelB'];
            }else{
                return $goods['levelB'];
            }
        }else{
            if(!empty($goods_images[2]) && !strstr($goods_images[2] ,'default')){
                return $goods_images[2];
            }elseif(!empty($goods_images[1]) && !strstr($goods_images[1] ,'default')){
                return $goods_images[1];
            }else{
                return $goods_images[1];
            }
        }
