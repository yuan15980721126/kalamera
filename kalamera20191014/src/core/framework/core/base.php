...
final class Base{
	...
	//	$setting['33hao_version'] = '<span class="vol"><font class="b">vinocave</font><font class="o">18</font></span>';
		$setting['33hao_version'] = '<span class="vol"><font class="b"></font><font class="o">1.0</font></span>';
	...
	private static function control(){
		...
		//二级域名
		if ($GLOBALS['setting_config']['enabled_subdomain'] == '1' && $_GET['model'] == 'index' && $_GET['fun'] == 'index'){
			$store_id = subdomain();
			if ($store_id > 0) $_GET['model'] = 'show_store';
		}
		$act_file = realpath(BASE_PATH.'/control/'.$_GET['model'].'.php');
		$class_name = $_GET['model'].'Control';
		if (!@include($act_file)){
		    if (C('debug')) {
		        throw_exception("Base Error: access file isn't exists!");
		    } else {
		        showMessage('Sorry! Page not found.','','html','error');
		    }
		}
		if (class_exists($class_name)){
			$main = new $class_name();
			$ops = $_GET['fun'] ? $_GET['fun'] : $_POST['fun'];
			$function = $ops .'Op';
			...
			$error = "Base Error: class2 $class_name isn't exists!";
			throw_exception($error);
		}
	}
private static function controladmin($admin){
		...
		$act_file = realpath(BASE_PATH.'/modules/'.$admin.'/control/'.$_GET['model'].'.php');
