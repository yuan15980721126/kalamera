...
function getSearchName() {
	...
	if (keyword == '') {
	    if(getCookie('deft_key_value') == null) {
	        $.getJSON(ApiUrl + '/index.php?model=index&fun=search_hot_info', function(result) {
...
function getFreeVoucher(tid) {
    ...
    $.ajax({
        type:'post',
        url:ApiUrl+"/index.php?model=member_voucher&fun=voucher_freeex",
...
function updateCookieCart(key) {
    ...
    if (cartlist) {
        $.ajax({
            type:'post',
            url:ApiUrl+'/index.php?model=member_cart&fun=cart_batchadd',
...
function getCartCount(key, expireHours) {
    ...
    if (getCookie('key') !== null && getCookie('cart_count') === null) {
        ...
        $.ajax({
            type:'post',
            url:ApiUrl+'/index.php?model=member_cart&fun=cart_count',
            data:{key:key},
            dataType:'json',
            async:false,
            success:function (result) {
...
function getChatCount() {
    if ($('#header').find('.message').length > 0) {
        var key = getCookie('key');
        if (key !== null) {
            $.getJSON(ApiUrl+'/index.php?model=member_chat&fun=get_msg_count', {key:key}, function(result){
                if (result.datas > 0) {
...
$(function() {
    $('.input-del').click(function(){
        $(this).parent().removeClass('write').find('input');
        $(this).prev().val('');
     ...
     */
        areaSelected: function(options) {
            ...
            function _getAreaList() {
                $.ajax({//获取区域列表
                    type:'get',
                    url:ApiUrl+'/index.php?model=area&fun=area_list',
                    data:{area_id:ASID},
                    dataType:'json',
                    async:false,
                    success:function(result){
...
function loadSeccode(){
    ...
    $.ajax({
        type:'get',
        url:ApiUrl+"/index.php?model=seccode&fun=makecodekey",
        async : false,
        dataType: 'json',
        success:function(result){
    ...
    rand =get_math('codekey');
    $("#shuffle").val(rand);
    
    // console.log(rand);
    $("#codeimage").attr('src',ApiUrl+'/index.php?model=seccode&fun=makecode&k='+$("#codekey").val()+'&t=' + rand);
    
}
function get_math(id){
    var ran = my_ran(1,1,10000);
    var rand =$("#"+id).val()+ran;
    return rand;
}
...
function favoriteStore(store_id){
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '/index.php?model=member_favorites_store&fun=favorites_add',
...
function dropFavoriteStore(store_id){
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '/index.php?model=member_favorites_store&fun=favorites_del',
        data: {key: key, store_id: store_id},
        dataType: 'json',
        async: false,
        success: function(result) {
...
function favoriteGoods(goods_id){
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '/index.php?model=member_favorites&fun=favorites_add',
