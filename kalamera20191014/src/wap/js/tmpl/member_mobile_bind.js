$(function() {
    ...
    $('#send').click(function(){
        if($.sValid()){
            ...
            $.ajax({
                type:'post',
                url:ApiUrl+"/index.php?model=member_account&fun=bind_mobile_step1",
                data:{key:key,mobile:mobile,captcha:captcha,codekey:shuffle},
                dataType:'json',
    ...
    $('#nextform').click(function(){
        ...
        if (auth_code) {
            $.ajax({
                type:'post',
                url:ApiUrl+"/index.php?model=member_account&fun=bind_mobile_step2",
                data:{key:key,auth_code:auth_code},
                dataType:'json',
                beforeSend:function(){ //触发ajax请求开始时执行
                    // $('#nextform').removeAttr('href');//去掉a标签中的href属性
