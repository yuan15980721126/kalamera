...
$(function(){
	...
	if(getQueryString('data-state')== 'state_ztui'){
		$("#filtrate_ul").css("left","-170px")
	}else if(getQueryString('data-state')== 'state_cancel'){
        $("#filtrate_ul").css("left","-210px");
    }
    var start_;
    var end_
	...
	function initPage(){
	    ...
	    var orderKey = $('#order_key').val();
    		...
    		$.ajax({
    			type:'post',
    			url:ApiUrl+"/index.php?model=member_order&fun=order_list&page="+page+"&curpage="+curpage,
    ...
    function cancelOrder(){
        ...
        $.sDialog({
            content: '<div class="cel_head">取消订单原因:</div>'+'<div><select id="state_info" name="state_info"><option id="d1" value="1">改买其他商品</option><option  id="d2" value="2">改用其他配送方式</option><option id="d4" value="3">其他原因</option></select></div>'+'<div class="s_tip">温馨提示：</div>'+'<div>1.订单取消后无法恢复</div>'+'<div>2.该订单已付金额将返还银行卡</div>',
            okFn: function() { cancelOrderId(order_id); }
        });
    }

    function cancelOrderId(order_id) {
        var select =$("#state_info").val();
        // console.log(select);return false;
        $.ajax({
            type:"post",
            url:ApiUrl+"/index.php?model=member_order&fun=order_cancel",
            data:{order_id:order_id,key:key,state_info:select},
            dataType:"json",
            success:function(result){
    ...
    function deleteOrderId(order_id) {
        $.ajax({
            type:"post",
            url:ApiUrl+"/index.php?model=member_order&fun=order_delete",
    ...
    $('#order-list').on('click','.agan_buy', function(){
...
function loaded () {
	myScroll = new IScroll('#fixed_nav', {
		scrollX: true,
		scrollY: false,
		momentum: false,
		snap: true,
